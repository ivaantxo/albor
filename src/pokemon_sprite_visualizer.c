...
{
    [BATTLE_BACKGROUND_NORMAL]   = _("NORMAL                  "),
    [BATTLE_BACKGROUND_GYM]      = _("GYM                     "),
    [BATTLE_BACKGROUND_MAGMA]    = _("MAGMA                   "),
    [BATTLE_BACKGROUND_AQUA]     = _("AQUA                    "),
    [BATTLE_BACKGROUND_SIDNEY]   = _("SIDNEY                  "),
    [BATTLE_BACKGROUND_PHOEBE]   = _("PHOEBE                  "),
    [BATTLE_BACKGROUND_GLACIA]   = _("GLACIA                  "),
    [BATTLE_BACKGROUND_DRAKE]    = _("DRAKE                   "),
    [BATTLE_BACKGROUND_LEADER]   = _("LEADER                  "),
    [BATTLE_BACKGROUND_WALLACE]  = _("WALLACE                 "),
    [BATTLE_BACKGROUND_GROUDON]  = _("GROUDON                 "),
    [BATTLE_BACKGROUND_KYOGRE]   = _("KYOGRE                  "),
    [BATTLE_BACKGROUND_RAYQUAZA] = _("RAYQUAZA                "),
};
...
static void BattleLoadOpponentMonSpriteGfxCustom(u16 species, bool8 isFemale, bool8 isShiny, u8 battlerId)
{
    ...
    // Paleta cruda: carga directa, sin descomprimir.
...
{
    ColocaSombraPokemon(shadowSprite, &gSprites[shadowSprite->sSombraDueno],
                        shadowSprite->tShadowXOffset, shadowSprite->tShadowYOffset);

    // Igual que en combate: la mezcla se reafirma cada fotograma, porque el resto
    // de la pantalla reescribe BLDCNT por su cuenta y sin segunda capa el
    // duplicado dejaria de mezclarse y saldria negro macizo.
...
static void LoadAndCreateEnemyShadowSpriteCustom(struct PokemonSpriteVisualizer *data, u16 species)
{
    ...
    data->frontShadowSpriteIdPrimary = CreaSombraPokemon(data->frontspriteId, 200);
    if (data->frontShadowSpriteIdPrimary >= MAX_SPRITES)
        return;

    sombra = &gSprites[data->frontShadowSpriteIdPrimary];
    sombra->tShadowXOffset = data->shadowSettings.overrideX;
...
static void LoadBattleBg(u8 battleBgType, u8 battleTerrain)
{
    ...
    case BATTLE_BACKGROUND_KYOGRE:
        ...
        break;
    case BATTLE_BACKGROUND_RAYQUAZA:
...
static void UpdateBattleBg(u8 taskId, bool8 increment)
{
    ...
    if (data->battleBgType == BATTLE_BACKGROUND_NORMAL)
    {
        if (increment)
        {
        ...
        else
        {
            if (data->battleTerrain == BATTLE_TERRAIN_GRASS)
                data->battleBgType = BATTLE_BACKGROUND_RAYQUAZA;
            else
                data->battleTerrain -= 1;
        }
    }
    else if (data->battleBgType == BATTLE_BACKGROUND_GYM)
    {
        if (increment)
            data->battleBgType += 1;
        else
        {
            data->battleBgType = BATTLE_BACKGROUND_NORMAL;
            data->battleTerrain = BATTLE_TERRAIN_PLAIN;
        }
    }
    else if (data->battleBgType == BATTLE_BACKGROUND_RAYQUAZA)
    {
        if (increment)
        {
            data->battleBgType = BATTLE_BACKGROUND_NORMAL;
...
// Vuelca el ajuste en una sola linea y con la forma exacta del macro de
// species_info, para poder copiarla tal cual. En dos lineas habia que emparejar
// registros consecutivos, y bastaba con que se colara otro log entre medias para
...
static void UpdateShadowSettingsValue(u8 taskId, bool8 increment)
{
    ...
    if (option == 0)
    {
        ...
        destino = &gSprites[data->frontShadowSpriteIdPrimary].tShadowXOffset;
    }
    else
    {
        offset = &data->shadowSettings.overrideY;
        destino = &gSprites[data->frontShadowSpriteIdPrimary].tShadowYOffset;
    }
...
static void ReloadPokemonSprites(struct PokemonSpriteVisualizer *data)
{
    ...
    DestruyeSombraPokemon(data->frontShadowSpriteIdPrimary);
