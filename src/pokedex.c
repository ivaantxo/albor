...
static bool8 LoadPokedexListPage(u8 page)
{
    ...
    default:
        ...
        IniciaFondosDesdePlantillas(MODO_0, sPokedex_BgTemplate, ARRAY_COUNT(sPokedex_BgTemplate));
        ...
        //CopyWindowToVram(WIN_POKEMON_LIST, COPIA_VENTANA_COMPLETA);
...
static u8 LoadInfoScreen(struct PokedexListItem *item, u8 monSpriteId)
{
    ...
    ResetBgsAndClearDma3BusyFlags();
    IniciaFondosDesdePlantillas(MODO_0, sInfoScreen_BgTemplate, ARRAY_COUNT(sInfoScreen_BgTemplate));
    SetBgTilemapBuffer(3, AllocZeroed(BG_SCREEN_SIZE));
    SetBgTilemapBuffer(2, AllocZeroed(BG_SCREEN_SIZE));
    SetBgTilemapBuffer(1, AllocZeroed(BG_SCREEN_SIZE));
    SetBgTilemapBuffer(0, AllocZeroed(BG_SCREEN_SIZE));
...
static void Task_LoadInfoScreen(u8 taskId)
{
    ...
    case 4:
        ...
        CopyWindowToVram(WIN_INFO, COPIA_VENTANA_COMPLETA);
        CopyBgTilemapBufferToVram(1);
        CopyBgTilemapBufferToVram(2);
        CopyBgTilemapBufferToVram(3);
        gMain.state++;
...
void Task_DisplayCaughtMonDexPageHGSS(u8 taskId)
{
    ...
    default:
        if (!gFundidoPaletas.activo)
        {
            ...
            ResetBgsAndClearDma3BusyFlags();
            IniciaFondosDesdePlantillas(MODO_0, sNewEntryInfoScreen_BgTemplate, ARRAY_COUNT(sNewEntryInfoScreen_BgTemplate));
            SetBgTilemapBuffer(3, AllocZeroed(BG_SCREEN_SIZE));
            SetBgTilemapBuffer(2, AllocZeroed(BG_SCREEN_SIZE));
...
static void Task_SwitchScreensFromStatsScreen(u8 taskId)
{
    if (!gFundidoPaletas.activo)
    {
        ...
        BorraIconoPokemon(&gSprites[gTasks[taskId].data[4]]); //Destroy pokemon icon sprite
...
static void EvoFormsPage_PrintNavigationButtons(void)
{
    ...
    CopyWindowToVram(WIN_NAVIGATION_BUTTONS, COPIA_VENTANA_COMPLETA);
}
    ...
    if (base_i < iterations)
    {
        LoadCompressedPalette(GetMonSpritePalFromSpeciesAndPersonality(targetSpecies, 0, 0), OBJ_PLTT_ID(5 + base_i), PLTT_SIZE_4BPP);
            if (isEevee)
                ...
                gTasks[taskId].data[5 + base_i] = CreaIconoPokemon(targetSpecies, 50 + 32*base_i, 31, 4, SIN_PERSONALIDAD); //Create pokemon sprite
...
static void Task_SwitchScreensFromEvolutionScreen(u8 taskId)
{
    ...
    if (!gFundidoPaletas.activo)
    {
        ...
        {
            BorraIconoPokemon(&gSprites[gTasks[taskId].data[4+i]]); //Destroy pokemon icon sprite
        }
        FreeAndDestroyMonPicSprite(gTasks[taskId].tMonSpriteId);
    ...
    for (i=0; i < 30; i++)
    {
        ...
        else
        {
            ...
            if (times < 7)
                ...
                gTasks[taskId].data[5 + times] = CreaIconoPokemon(speciesForm, 18 + 34 * (times - 7), 70, 4, SIN_PERSONALIDAD); //Create pokemon sprite
