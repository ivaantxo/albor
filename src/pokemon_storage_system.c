...
static void LoadItemIconGfx(u8, const u32 *, const u16 *);
...
void DrawTextWindowAndBufferTiles(const u8 *string, void *dst, u8 zero1, u8 zero2, s32 bytesToBuffer)
{
    ...
    AddTextPrinterParameterized4(windowId, FUENTE_NORMAL, 0, 1, 0, 0, txtColor, TEXT_SKIP_DRAW, string);
...
static void Task_PCMainMenu(u8 taskId)
{
    ...
    case STATE_LOAD:
        ...
        AddTextPrinterParameterized2(0, FUENTE_NORMAL, sMainMenuTexts[task->tSelectedOption].desc, TEXT_SKIP_DRAW, NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
    ...
    case STATE_HANDLE_INPUT:
        ...
        case MENU_NOTHING_CHOSEN:
            ...
            if (task->tSelectedOption != task->tNextOption)
            {
                ...
                FillWindowPixelBuffer(0, PIXEL_FILL(1));
                AddTextPrinterParameterized2(0, FUENTE_NORMAL, sMainMenuTexts[task->tSelectedOption].desc, 0, NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
            }
            break;
        case MENU_B_PRESSED:
        ...
        default:
            if (task->tInput == OPTION_WITHDRAW && CountPartyMons() == PARTY_SIZE)
            {
                // Can't withdraw
                FillWindowPixelBuffer(0, PIXEL_FILL(1));
                AddTextPrinterParameterized2(0, FUENTE_NORMAL, gText_PartyFull, 0, NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
            ...
            else if (task->tInput == OPTION_DEPOSIT && CountPartyMons() == 1)
            {
                // Can't deposit
                FillWindowPixelBuffer(0, PIXEL_FILL(1));
                AddTextPrinterParameterized2(0, FUENTE_NORMAL, gText_JustOnePkmn, 0, NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
                task->tState = STATE_ERROR_MSG;
            }
            else
            {
...
static void ChooseBoxMenu_PrintInfo(void)
{
    ...
    // Print box name
    center = GetStringCenterAlignXOffset(FUENTE_NORMAL, boxName, 64);
    AddTextPrinterParameterized3(windowId, FUENTE_NORMAL, center, 1, sChooseBoxMenu_TextColors, TEXT_SKIP_DRAW, boxName);
    ...
    center = GetStringCenterAlignXOffset(FUENTE_NORMAL, numBoxMonsText, 64);
    AddTextPrinterParameterized3(windowId, FUENTE_NORMAL, center, 17, sChooseBoxMenu_TextColors, TEXT_SKIP_DRAW, numBoxMonsText);
...
static void LoadDisplayMonGfx(u16 species, u32 pid)
{
    ...
    if (species == SPECIES_EGG)
    {
        ...
        // Las dos mitades de la paleta del huevo van crudas: se cargan desde la ROM
        // sin buffer intermedio.
        LoadPalette(pal1->data, sStorage->displayMonPalOffset, PLTT_SIZE_4BPP / 2);
        LoadPalette(pal2->data, sStorage->displayMonPalOffset + 8, PLTT_SIZE_4BPP / 2);
...
static void PrintDisplayMonInfo(void)
{
    ...
    if (sStorage->boxOption != OPTION_MOVE_ITEMS)
    {
        AddTextPrinterParameterized(WIN_DISPLAY_INFO, GetFontIdToFit(sStorage->displayMonNameText, FUENTE_NORMAL, 0, WindowWidthPx(WIN_DISPLAY_INFO) - 6), sStorage->displayMonNameText, 6, 0, TEXT_SKIP_DRAW, NULL);
        AddTextPrinterParameterized(WIN_DISPLAY_INFO, GetFontIdToFit(sStorage->displayMonNameText, FUENTE_NORMAL, 0, WindowWidthPx(WIN_DISPLAY_INFO) - 12), sStorage->displayMonSpeciesName, 6, 15, TEXT_SKIP_DRAW, NULL);
        AddTextPrinterParameterized(WIN_DISPLAY_INFO, FUENTE_NORMAL, sStorage->displayMonGenderLvlText, 10, 29, TEXT_SKIP_DRAW, NULL);
        AddTextPrinterParameterized(WIN_DISPLAY_INFO, GetFontIdToFit(sStorage->displayMonItemName, FUENTE_NORMAL, 0, WindowWidthPx(WIN_DISPLAY_INFO) - 6), sStorage->displayMonItemName, 6, 43, TEXT_SKIP_DRAW, NULL);
    }
    else
    {
        AddTextPrinterParameterized(WIN_DISPLAY_INFO, GetFontIdToFit(sStorage->displayMonItemName, FUENTE_NORMAL, 0, WindowWidthPx(WIN_DISPLAY_INFO) - 6), sStorage->displayMonItemName, 6, 0, TEXT_SKIP_DRAW, NULL);
        AddTextPrinterParameterized(WIN_DISPLAY_INFO, GetFontIdToFit(sStorage->displayMonNameText, FUENTE_NORMAL, 0, WindowWidthPx(WIN_DISPLAY_INFO) - 6), sStorage->displayMonNameText, 6, 13, TEXT_SKIP_DRAW, NULL);
        AddTextPrinterParameterized(WIN_DISPLAY_INFO, GetFontIdToFit(sStorage->displayMonSpeciesName, FUENTE_NORMAL, 0, WindowWidthPx(WIN_DISPLAY_INFO) - 12), sStorage->displayMonSpeciesName, 6, 28, TEXT_SKIP_DRAW, NULL);
        AddTextPrinterParameterized(WIN_DISPLAY_INFO, FUENTE_NORMAL, sStorage->displayMonGenderLvlText, 10, 42, TEXT_SKIP_DRAW, NULL);
    }
...
static void PrintMessage(u8 id)
{
    ...
    AddTextPrinterParameterized(WIN_MESSAGE, FUENTE_NORMAL, sStorage->messageText, 0, 1, TEXT_SKIP_DRAW, NULL);
...
static void SetBoxMonDynamicPalette(u32 boxId, u32 position)
{
    ...
    {
        if (species == SPECIES_CASTFORM) // REVISAR SI ESTO ES NECESARIO
        {
            CopiaCpu16(palette, &sPaletteSwapBuffer[PLTT_ID(position)], PLTT_SIZE_4BPP);
        }
        else
        {
            CopiaCpu16(palette, &sPaletteSwapBuffer[PLTT_ID(position)], PLTT_SIZE_4BPP);
            DesplazaTonoPaletaBuffer(&sPaletteSwapBuffer[PLTT_ID(position)], personality);
        }
    }
...
static void CreatePartyMonsSprites(bool8 visible)
{
    ...
    {
        LoadPalette(GetMonFrontSpritePal(&gPlayerParty[0]), OBJ_PLTT_ID(1), PLTT_SIZE_4BPP);
    ...
    for (i = 1; i < PARTY_SIZE; i++)
    {
        ...
        else
        {
            if (species != SPECIES_NONE)
            {
                ...
                LoadPalette(GetMonFrontSpritePal(&gPlayerParty[i]), OBJ_PLTT_ID(paletteNum), PLTT_SIZE_4BPP);
...
static void SetPlacedMonSprite(u8 boxId, u8 position)
{
    ...
    if (boxId == TOTAL_BOXES_COUNT) // party mon
    {
        ...
        else // If currently using displayed mon palette, load party sprite palette into free party palette slot
        {
            ...
            {
                paletteNum = FindFreePartyPaletteSlot();
                LoadPalette(GetMonFrontSpritePal(&gPlayerParty[position]), OBJ_PLTT_ID(paletteNum), PLTT_SIZE_4BPP);
...
{
    return ANCHO_PANTALLA - 64 - GetStringWidth(FUENTE_NORMAL, string, 0) / 2;
}
...
static void GetRestrictedReleaseMoves(u16 *moves)
{
    ...
    {
        if (sRestrictedReleaseMoves[i].mapGroup == MAP_GROUPS_COUNT || (sRestrictedReleaseMoves[i].mapGroup == gSaveBlockPtr->location.mapGroup && sRestrictedReleaseMoves[i].mapNum == gSaveBlockPtr->location.mapNum))
        {
...
static bool8 IsMovingItem(void)
{
    ...
    if (sStorage->boxOption == OPTION_MOVE_ITEMS)
    {
        for (i = 0; i < MAX_ITEM_ICONS; i++)
        {
            if (sStorage->itemIcons[i].active && sStorage->itemIcons[i].area == CURSOR_AREA_BOX_TITLE)
                return TRUE;
        }
    }
    return FALSE;
...
static void LoadItemIconGfx(u8 id, const u32 *itemTiles, const u16 *itemPal)
{
    ...
    // La paleta del objeto va cruda: copia directa.
    CopiaCpu16(itemPal, sStorage->itemIconBuffer, PLTT_SIZE_4BPP);
...
static void PrintItemDescription(void)
{
    ...
    AddTextPrinterParameterized5(WIN_ITEM_DESC, FUENTE_NORMAL, description, 4, 0, 0, NULL, 0, 1);
}
...
u32 CountStorageNonEggMons(void)
{
    ...
    for (i = 0; i < TOTAL_BOXES_COUNT; i++)
    {
        for (j = 0; j < IN_BOX_COUNT; j++)
        {
            if (GetBoxMonData(&gPokemonStoragePtr->boxes[i][j], MON_DATA_SPECIES) && !GetBoxMonData(&gPokemonStoragePtr->boxes[i][j], MON_DATA_IS_EGG))
                count++;
        }
    }
...
bool32 AnyStorageMonWithMove(u16 moveId)
{
    ...
    for (i = 0; i < TOTAL_BOXES_COUNT; i++)
    {
        for (j = 0; j < IN_BOX_COUNT; j++)
        {
            if (GetBoxMonData(&gPokemonStoragePtr->boxes[i][j], MON_DATA_SPECIES) && !GetBoxMonData(&gPokemonStoragePtr->boxes[i][j], MON_DATA_IS_EGG) && GetBoxMonData(&gPokemonStoragePtr->boxes[i][j], MON_DATA_KNOWN_MOVES, (u8 *)moves))
                return TRUE;
        }
    }
