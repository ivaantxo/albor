...
static void Cmd_jumpifataquefallo(void);
static void Cmd_ponresultadoataque(void);
static void Cmd_jumpifataquenoafecta(void);
static void Cmd_MarcaBajadaPorIntimidacion(void);
static void Cmd_MarcaCambioEstadisticaSilencioso(void);
static void Cmd_EscribeTextoEstadoAlterado(void);
static void Cmd_EscribeTextoPrevencionEstado(void);
static void Cmd_MarcaPrevencionPorHabilidadPropia(void);
static void Cmd_EscribeTextoCambioClima(void);
static void Cmd_EscribeTextoClimaContinua(void);
static void Cmd_EscribeTextoDanioClima(void);
static void Cmd_EscribeTextoClimaTermina(void);
static void Cmd_EscribeTextoLluvia(void);
static void Cmd_EscribeTextoNeblina(void);
static void Cmd_EscribeTextoSustituto(void);
static void Cmd_EscribeTextoReserva(void);
static void Cmd_EscribeTextoTragarFalla(void);
static void Cmd_EscribeTextoDescanso(void);
static void Cmd_EscribeTextoAbsorbeFuego(void);
static void Cmd_EscribeTextoBaya(void);
static void Cmd_EscribeTextoDrenaje(void);
static void Cmd_EscribeTextoDrenadorasTurno(void);
static void Cmd_EscribeTextoDrenadoras(void);
static void Cmd_EscribeTextoCambioObjeto(void);
static void Cmd_EscribeTextoHabilidadEntrada(void);
static void Cmd_EscribeTextoHierbaMental(void);
static void Cmd_EscribeTextoBarrera(void);
static void Cmd_EscribeTextoCuraEquipo(void);
static void Cmd_jumpifinsonorizadoatacante(void);
static void Cmd_jumpifinsonorizadoaliado(void);
static void Cmd_EscribeTextoAtrapado(void);
static void Cmd_EscribeTextoDanioTrampa(void);
static void Cmd_EscribeTextoBallEscapa(void);
static void Cmd_EscribeTextoTrampaPuesta(void);
static void Cmd_EscribeTextoEntraPokemon(void);
static void Cmd_EscribeTextoDevolverPokemon(void);
static void Cmd_settypedhazard(void);
static void Cmd_ponmarca(void);
static void Cmd_quitamarca(void);
static void Cmd_jumpifmarca(void);
...
static void Cmd_jumpifestado(void);
static void Cmd_jumpiftransitorio(void);
...
static void Cmd_animatransitorio(void);
...
static void Cmd_pontransitorio(void);
...
void (*const gBattleScriptingCommandsTable[])(void) =
    {
        ...
        BATTLE_CMD(jumpifestado),
        BATTLE_CMD(jumpiftransitorio),
        ...
        BATTLE_CMD(jumpifataquefallo),
        BATTLE_CMD(ponresultadoataque),
        BATTLE_CMD(jumpifataquenoafecta),
        BATTLE_CMD(MarcaBajadaPorIntimidacion),
        BATTLE_CMD(MarcaCambioEstadisticaSilencioso),
        BATTLE_CMD(EscribeTextoEstadoAlterado),
        BATTLE_CMD(EscribeTextoPrevencionEstado),
        BATTLE_CMD(MarcaPrevencionPorHabilidadPropia),
        BATTLE_CMD(EscribeTextoCambioClima),
        BATTLE_CMD(EscribeTextoClimaContinua),
        BATTLE_CMD(EscribeTextoDanioClima),
        BATTLE_CMD(EscribeTextoClimaTermina),
        BATTLE_CMD(EscribeTextoLluvia),
        BATTLE_CMD(EscribeTextoNeblina),
        BATTLE_CMD(EscribeTextoSustituto),
        BATTLE_CMD(EscribeTextoReserva),
        BATTLE_CMD(EscribeTextoTragarFalla),
        BATTLE_CMD(EscribeTextoDescanso),
        BATTLE_CMD(EscribeTextoAbsorbeFuego),
        BATTLE_CMD(EscribeTextoBaya),
        BATTLE_CMD(EscribeTextoDrenaje),
        BATTLE_CMD(EscribeTextoDrenadorasTurno),
        BATTLE_CMD(EscribeTextoDrenadoras),
        BATTLE_CMD(EscribeTextoCambioObjeto),
        BATTLE_CMD(EscribeTextoHabilidadEntrada),
        BATTLE_CMD(EscribeTextoHierbaMental),
        BATTLE_CMD(EscribeTextoBarrera),
        BATTLE_CMD(EscribeTextoCuraEquipo),
        BATTLE_CMD(jumpifinsonorizadoatacante),
        BATTLE_CMD(jumpifinsonorizadoaliado),
        BATTLE_CMD(EscribeTextoAtrapado),
        BATTLE_CMD(EscribeTextoDanioTrampa),
        BATTLE_CMD(EscribeTextoBallEscapa),
        BATTLE_CMD(EscribeTextoTrampaPuesta),
        BATTLE_CMD(EscribeTextoEntraPokemon),
        BATTLE_CMD(EscribeTextoDevolverPokemon),
        BATTLE_CMD(settypedhazard),
        BATTLE_CMD(ponmarca),
        BATTLE_CMD(quitamarca),
        BATTLE_CMD(jumpifmarca),
};
...
// Que estado permanente pone cada efecto de movimiento.
static const u32 sStatusFlagsForMoveEffects[NUM_MOVE_EFFECTS] =
    {
        [MOVE_EFFECT_SLEEP] = ESTADO_SUENO,
        [MOVE_EFFECT_POISON] = ESTADO_VENENO,
        [MOVE_EFFECT_BURN] = ESTADO_QUEMADURA,
        [EFECTO_MOVIMIENTO_CONGELACION] = ESTADO_CONGELACION,
        [MOVE_EFFECT_PARALYSIS] = ESTADO_PARALISIS,
        [MOVE_EFFECT_TOXIC] = ESTADO_VENENO,
};

// Y que estado transitorio. Se guarda +1 para que el cero signifique "ninguno".
static const u8 sTransitorioPorEfecto[NUM_MOVE_EFFECTS] =
    {
        [MOVE_EFFECT_CONFUSION] = TRANSITORIO_CONFUSION + 1,
        [MOVE_EFFECT_FLINCH] = TRANSITORIO_SORPRESA + 1,
        [MOVE_EFFECT_CHARGING] = TRANSITORIO_MULTITURNO + 1,
        [MOVE_EFFECT_WRAP] = TRANSITORIO_SIN_ESCAPE + 1,
        [MOVE_EFFECT_RECHARGE] = TRANSITORIO_DESCANSO + 1,
        [MOVE_EFFECT_PREVENT_ESCAPE] = TRANSITORIO_SIN_ESCAPE + 1,
        [MOVE_EFFECT_THRASH] = TRANSITORIO_ENCADENADO + 1,
};
...
static void RecuerdaHabilidadCombate(u32 battler, u32 ability)
{
    AI_DATA->abilities[battler] = ability;
}

static bool32 IsBattlerWeatherAffected(u32 battler, u32 weatherFlags)
{
    if (!ClimaTieneEfecto())
        return FALSE;
    if (GetBattlerHoldEffect(battler, TRUE) == HOLD_EFFECT_UTILITY_UMBRELLA)
        return FALSE;
    return (1u << ObtenClimaCombate()) & weatherFlags;
}
...
static void Cmd_attackcanceler(void)
{
    ...
    if (!IsBattlerAlive(gBattlerAttacker) && gMovimientos[gCurrentMove].effect != EFFECT_EXPLOSION && !(gMarcasGolpe[MARCA_SIN_TEXTO_ATAQUE]))
    {
    ...
    if (!gBattleMons[gBattlerAttacker].pp[gCurrMovePos] && gCurrentMove != MOVE_STRUGGLE && !((gMarcasGolpe[MARCA_PERMITIR_SIN_PP] || gMarcasGolpe[MARCA_SIN_TEXTO_ATAQUE] || gMarcasGolpe[MARCA_SIN_GASTAR_PP])) && !(TransitorioActivo(gBattlerAttacker, TRANSITORIO_MULTITURNO)))
    {
        gBattlescriptCurrInstr = BattleScript_NoPPForMove;
        gCombate->resultadoAtaque = RESULTADO_MOVIMIENTO_FALLO;
        return;
    }

    gMarcasGolpe[MARCA_PERMITIR_SIN_PP] = FALSE;

    // Check if no available target present on the field or if Sky Battles ban the move
    if ((NoTargetPresent(gBattlerAttacker, gCurrentMove) && (!gBattleMoveEffects[gMovimientos[gCurrentMove].effect].twoTurnEffect || (TransitorioActivo(gBattlerAttacker, TRANSITORIO_MULTITURNO)))))
    {
        gBattlescriptCurrInstr = BattleScript_FailedFromAtkString;

        if (!gBattleMoveEffects[gMovimientos[gCurrentMove].effect].twoTurnEffect || (TransitorioActivo(gBattlerAttacker, TRANSITORIO_MULTITURNO)))
    ...
    else if (IsBattlerProtected(gBattlerAttacker, gBattlerTarget, gCurrentMove) && (gCurrentMove != MOVE_CURSE || EsTipo(gBattlerAttacker, TIPO_FANTASMA)) && (!gBattleMoveEffects[gMovimientos[gCurrentMove].effect].twoTurnEffect || (TransitorioActivo(gBattlerAttacker, TRANSITORIO_MULTITURNO))) && gMovimientos[gCurrentMove].effect != EFFECT_GOLPE_BAJO && gMovimientos[gCurrentMove].effect != EFFECT_PALMA_RAUDA)
    {
        CancelMultiTurnMoves(gBattlerAttacker);
        gCombate->resultadoAtaque = RESULTADO_MOVIMIENTO_FALLO;
        ...
        gCombate->textoResultadoOverride = COMPOUND_STRING("¡{B_DEF_NAME_WITH_PREFIX} se protegió!");
...
u32 GetTotalAccuracy(u32 battlerAtk, u32 battlerDef, u32 move, u32 atkAbility, u32 defAbility, u32 atkHoldEffect, u32 defHoldEffect)
{
    ...
    case ABILITY_TANGLED_FEET:
        if (TransitorioActivo(battlerDef, TRANSITORIO_CONFUSION))
            MULTIPLICA(modifier, MENOS_25_POR_CIENTO);
        break;
...
static void AccuracyCheck(bool32 recalcDragonDarts, const u8 *nextInstr, const u8 *failInstr, u16 move)
{
    ...
    {
        if (ES_SEMI_INVULNERABLE(gBattlerTarget))
    ...
    else
    {
        ...
        if (!PorcentajeAleatorio(accuracy))
        {
                ...
                && CanTargetPartner(gBattlerAttacker, gBattlerTarget) && !TargetFullyImmuneToCurrMove(gBattlerAttacker, ALIADO(gBattlerTarget)))
            {
                ...
                gCombate->resultadoAtaque = RESULTADO_MOVIMIENTO_CONECTA;
            ...
            if (EsCombateContraEntrenador(gCombate->tipoCombate) &&
                ...
                gCombate->textoResultadoOverride = COMPOUND_STRING("¡{B_DEF_NAME_WITH_PREFIX} esquivó el ataque!");
            else
                gCombate->textoResultadoOverride = COMPOUND_STRING("¡El ataque de {B_ATK_NAME_WITH_PREFIX}  falló!");
...
static void Cmd_attackstring(void)
{
    ...
    if (!((gMarcasGolpe[MARCA_SIN_TEXTO_ATAQUE] || gMarcasGolpe[MARCA_TEXTO_ATAQUE_ESCRITO])))
    {
        EscribeTextoCombate(gBattlerAttacker, COMPOUND_STRING("¡{B_ATK_NAME_WITH_PREFIX} usó {B_CURRENT_MOVE}!"));
        gMarcasGolpe[MARCA_TEXTO_ATAQUE_ESCRITO] = TRUE;
    }
    gBattlescriptCurrInstr = cmd->nextInstr;
...
static void Cmd_ppreduce(void)
{
    ...
    if (TransitorioActivo(gBattlerAttacker, TRANSITORIO_MULTITURNO))
        gMarcasGolpe[MARCA_SIN_GASTAR_PP] = TRUE;

    if (!((gMarcasGolpe[MARCA_SIN_GASTAR_PP] || gMarcasGolpe[MARCA_SIN_TEXTO_ATAQUE])) && gBattleMons[gBattlerAttacker].pp[gCurrMovePos])
    {
...
s32 CalcCritChanceStageArgs(u32 battlerAtk, u32 battlerDef, u32 move, bool32 recordAbility, u32 abilityAtk, u32 abilityDef, u32 holdEffectAtk)
{
    ...
    if (gMovimientos[move].alwaysCriticalHit || (abilityAtk == ABILITY_MERCILESS && EstadoActivo(battlerDef, ESTADO_VENENO)))
    {
        critChance = -2;
    }
    else
...
static void Cmd_attackanimation(void)
{
    ...
    if ((gMarcasGolpe[MARCA_SIN_ANIMACION]) && gCurrentMove != MOVE_SUBSTITUTE)
    {
    ...
    else
    {
        ...
        if (MovimientoEsEfectivo(gCombate->resultadoMovimiento))
        {
            ...
            if (TransitorioActivo(gBattlerTarget, TRANSITORIO_SUSTITUTO))
...
static void Cmd_healthbarupdate(void)
{
    ...
    if (MovimientoEsEfectivo(gCombate->resultadoMovimiento) || (gMarcasGolpe[MARCA_DANIO_PASIVO]))
    {
        u32 battler = GetBattlerForBattleScript(cmd->battler);

        if (DoesSubstituteBlockMove(gBattlerAttacker, battler, gCurrentMove) && gDisableStructs[battler].substituteHP && !(gMarcasGolpe[MARCA_IGNORA_SUSTITUTO]))
        {
            EscribeTextoCombate(battler, COMPOUND_STRING("The SUBSTITUTE took damage for {B_DEF_NAME_WITH_PREFIX}!"));
        }
    ...
    if (MovimientoEsEfectivo(gCombate->resultadoMovimiento) || (gMarcasGolpe[MARCA_DANIO_PASIVO]))
    {
        ...
        else
        {
            ...
            else
            {
                ...
                gCombate->danioRecibido[battler] = gHpDealt;
            }
...
static void Cmd_critmessage(void)
{
    ...
    if (!HayAlgunCombatienteOcupado())
    {
        ...
        {
            EscribeTextoCombate(gBattlerAttacker, COMPOUND_STRING("¡Un golpe crítico!"));
...
static void Cmd_HazSonidoEfectividad(void)
{
    ...
    if (gCombate->resultadoAtaque == RESULTADO_MOVIMIENTO_CONECTA)
    {
...
static void Cmd_resultmessage(void)
{
    ...
    if (gCombate->textoResultadoOverride != NULL)
    {
        texto = gCombate->textoResultadoOverride;
        gCombate->textoResultadoOverride = NULL;
    }
    else switch (gCombate->resultadoAtaque)
    {
    case RESULTADO_MOVIMIENTO_CONECTA:
        if (!gCombate->contadorMultigolpes) // Don't print effectiveness on each hit in a multi hit attack
        {
            if (gCombate->resultadoMovimiento > MOVIMIENTO_NEUTRO)
            {
                ...
                texto = COMPOUND_STRING("¡Es super efectivo!");
            }
            else if (gCombate->resultadoMovimiento < MOVIMIENTO_NEUTRO && gCombate->resultadoMovimiento != MOVIMIENTO_NO_EFECTIVO)
            {
                texto = COMPOUND_STRING("No es muy efectivo…");
            }
        }
        break;
    case RESULTADO_MOVIMIENTO_FALLIDO:
        texto = COMPOUND_STRING("¡Pero falló!");
        break;
    case RESULTADO_MOVIMIENTO_NO_AFECTA:
        texto = COMPOUND_STRING("It doesn't affect {B_DEF_NAME_WITH_PREFIX}…");
        break;
    default: // RESULTADO_MOVIMIENTO_FALLO sin texto propio (ya asignado en su origen si lo necesitaba)
        break;
    }
...
static void Cmd_EscribeTextoEntraPokemon(void)
{
    ...
    if (!HayAlgunCombatienteOcupado())
    {
        EscribeTextoEntraPokemon(GetBattlerForBattleScript(cmd->battler));
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
}
...
static void Cmd_EscribeTextoDevolverPokemon(void)
{
    ...
    if (!HayAlgunCombatienteOcupado())
    {
        EscribeTextoDevolverPokemon(GetBattlerForBattleScript(cmd->battler));
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
}
...
// Sustituye a `jumpifword ... gMensajeBatalla, TEXTO_COMBATE_PROTECCION`. Se usa
// justo tras attackcanceler, para saltar si el movimiento ya ha sido bloqueado
// (por Protección u otra causa) antes de gastar el turno en calcular daño.
static void Cmd_jumpifataquefallo(void)
{
    CMD_ARGS(const u8 *jumpInstr);

    if (gCombate->resultadoAtaque == RESULTADO_MOVIMIENTO_FALLO)
        gBattlescriptCurrInstr = cmd->jumpInstr;
    else
        gBattlescriptCurrInstr = cmd->nextInstr;
}

// Sustituye a `orhalfword/bichalfword/sethword gMoveResultFlags, MOVE_RESULT_*` en los scripts.
static void Cmd_ponresultadoataque(void)
{
    CMD_ARGS(u8 valor);

    gCombate->resultadoAtaque = cmd->valor;
    gBattlescriptCurrInstr = cmd->nextInstr;
}

// Sustituye a `jumpifbyte ... gMoveResultFlags, MOVE_RESULT_NO_EFFECT` (jumpifmovehadnoeffect).
static void Cmd_jumpifataquenoafecta(void)
{
    CMD_ARGS(const u8 *jumpInstr);

    if (gCombate->resultadoAtaque == RESULTADO_MOVIMIENTO_NO_AFECTA)
        gBattlescriptCurrInstr = cmd->jumpInstr;
    else
        gBattlescriptCurrInstr = cmd->nextInstr;
}
...
// Sustituye a las cinco tablas gGot*StringIds / gFellAsleepStringIds, que tenían
// todas la misma forma: el mismo mensaje cambia según lo cause un movimiento o una
// habilidad. La congelación no llegó a tener tabla ni texto; se le da uno aquí.
static void Cmd_EscribeTextoEstadoAlterado(void)
{
    CMD_ARGS(u8 estado);
    ...
    bool32 porHabilidad = gEstadoAlteradoPorHabilidad;
    const u8 *texto = NULL;

    switch (cmd->estado)
    {
    case MOVE_EFFECT_SLEEP:
        texto = porHabilidad ? COMPOUND_STRING("{B_SCR_ACTIVE_NAME_WITH_PREFIX}'s {B_BUFF1} made {B_EFF_NAME_WITH_PREFIX} sleep!")
                             : COMPOUND_STRING("{B_EFF_NAME_WITH_PREFIX} fell asleep!");
        break;
    case MOVE_EFFECT_POISON:
        texto = porHabilidad ? COMPOUND_STRING("¡{B_EFF_NAME_WITH_PREFIX} fue envenenado por {B_BUFF1} de {B_SCR_ACTIVE_NAME_WITH_PREFIX}!")
                             : COMPOUND_STRING("¡{B_EFF_NAME_WITH_PREFIX} fue envenenado!");
        break;
    case MOVE_EFFECT_BURN:
        texto = porHabilidad ? COMPOUND_STRING("¡{B_SCR_ACTIVE_ABILITY} de {B_SCR_ACTIVE_NAME_WITH_PREFIX} ha quemado a {B_EFF_NAME_WITH_PREFIX}!")
                             : COMPOUND_STRING("¡{B_EFF_NAME_WITH_PREFIX} fue quemado!");
        break;
    case EFECTO_MOVIMIENTO_CONGELACION:
        texto = porHabilidad ? COMPOUND_STRING("¡{B_SCR_ACTIVE_ABILITY} de {B_SCR_ACTIVE_NAME_WITH_PREFIX} ha congelado a {B_EFF_NAME_WITH_PREFIX}!")
                             : COMPOUND_STRING("¡{B_EFF_NAME_WITH_PREFIX} fue congelado!");
        break;
    case MOVE_EFFECT_PARALYSIS:
        texto = porHabilidad ? COMPOUND_STRING("{B_SCR_ACTIVE_NAME_WITH_PREFIX}'s {B_BUFF1} paralyzed {B_EFF_NAME_WITH_PREFIX}!\lIt may be unable to move!")
                             : COMPOUND_STRING("{B_EFF_NAME_WITH_PREFIX} is paralyzed! It may be unable to move!");
        break;
    }
...
// Sustituye a gBRN/gPRLZ/gPSNPreventionStringIds. Las tres tablas compartían dos de
// sus tres entradas; lo único propio de cada estado es el primer mensaje.
static void Cmd_EscribeTextoPrevencionEstado(void)
{
    CMD_ARGS(u8 estado);
    ...
    switch (gResultadoPrevencionEstado)
    {
    case PREVENCION_POR_HABILIDAD_PROPIA:
        switch (cmd->estado)
        {
        case MOVE_EFFECT_BURN:
            texto = COMPOUND_STRING("{B_EFF_NAME_WITH_PREFIX}'s {B_EFF_ABILITY} prevents burns!");
            break;
        case MOVE_EFFECT_PARALYSIS:
            texto = COMPOUND_STRING("{B_EFF_NAME_WITH_PREFIX}'s {B_DEF_ABILITY} prevents paralysis!");
            break;
        case MOVE_EFFECT_POISON:
            texto = COMPOUND_STRING("{B_EFF_NAME_WITH_PREFIX}'s {B_DEF_ABILITY} prevents poisoning!");
            break;
        }
        break;
    case PREVENCION_POR_HABILIDAD_ATACANTE:
        texto = COMPOUND_STRING("{B_ATK_NAME_WITH_PREFIX}'s {B_ATK_ABILITY} prevents {B_DEF_NAME_WITH_PREFIX}'s {B_DEF_ABILITY} from working!");
        break;
    case PREVENCION_SIN_EFECTO:
        texto = COMPOUND_STRING("{B_SCR_ACTIVE_NAME_WITH_PREFIX}'s {B_SCR_ACTIVE_ABILITY} had no effect on {B_EFF_NAME_WITH_PREFIX}!");
        break;
    }
...
// Los cuatro comandos de clima que siguen sustituyen a gMoveWeatherChangeStringIds,
// gSandStormHailSnowContinuesStringIds, gSandStormHailDmgStringIds y
// gSandStormHailSnowEndStringIds. Ninguno necesita variable intermedia: el clima ya
// es estado, así que basta con mirarlo.
//
// El granizo ya no existe como clima propio en esta base (lo sustituyó la nieve), así
// que sus entradas de las tablas viejas se quedan fuera.
...
static void Cmd_EscribeTextoCambioClima(void)
{
    ...
    {
        switch (gCombate->clima.modo)
        {
        case CLIMA_COMBATE_LLUVIA: texto = COMPOUND_STRING("¡Ha empezado a llover!");     break;
        case CLIMA_COMBATE_SOL:    texto = COMPOUND_STRING("¡El Sol ha empezado a brillar!"); break;
        case CLIMA_COMBATE_ARENA:  texto = COMPOUND_STRING("A sandstorm kicked up!");     break;
        case CLIMA_COMBATE_NIEVE:  texto = COMPOUND_STRING("¡Ha empezado a nevar!");      break;
        default: break;
        }
    }
...
static void Cmd_EscribeTextoClimaContinua(void)
{
    ...
    switch (gCombate->clima.modo)
    {
    case CLIMA_COMBATE_ARENA: texto = COMPOUND_STRING("The sandstorm is raging."); break;
    case CLIMA_COMBATE_NIEVE: texto = COMPOUND_STRING("La nieve sigue cayendo");   break;
    default: break;
    }
...
static void Cmd_EscribeTextoDanioClima(void)
{
    ...
    switch (gCombate->clima.modo)
    {
    case CLIMA_COMBATE_ARENA: texto = COMPOUND_STRING("{B_ATK_NAME_WITH_PREFIX} is buffeted by the sandstorm!"); break;
    // La nieve heredó el mensaje que tenía el granizo.
    case CLIMA_COMBATE_NIEVE: texto = COMPOUND_STRING("{B_ATK_NAME_WITH_PREFIX} is pelted by HAIL!");            break;
    default: break;
    }
...
static void Cmd_EscribeTextoClimaTermina(void)
{
    ...
    switch (gCombate->clima.modo)
    {
    case CLIMA_COMBATE_ARENA: texto = COMPOUND_STRING("The sandstorm subsided."); break;
    case CLIMA_COMBATE_NIEVE: texto = COMPOUND_STRING("La nieve ha parado.");     break;
    default: break;
    }
...
// Sustituye a gRainContinuesStringIds. Para cuando se imprime, la lluvia ya se ha
// quitado del clima si tocaba terminar, así que el propio clima lo dice.
static void Cmd_EscribeTextoLluvia(void)
{
    CMD_ARGS();
    ...
    const u8 *texto = (gCombate->clima.modo == CLIMA_COMBATE_LLUVIA)
                    ? COMPOUND_STRING("Sigue lloviendo…")
                    : COMPOUND_STRING("The rain stopped.");

    gBattlescriptCurrInstr = cmd->nextInstr;
    EscribeTextoCombate(gBattlerAttacker, texto);
}

// Los comandos que siguen sustituyen a una tabla g*StringIds cada uno. Cuando el
// resultado ya queda registrado en el estado del combate, se lee de ahí y no hace
// falta ninguna variable; solo se guarda una bandera cuando el propio código pisa
// después el dato que haría falta.
...
static void Cmd_EscribeTextoNeblina(void)
{
    ...
    const u8 *texto = (gCombate->resultadoAtaque == RESULTADO_MOVIMIENTO_FALLIDO)
                    ? COMPOUND_STRING("¡Pero falló!")
                    : COMPOUND_STRING("{B_ATK_PREFIX2} became shrouded in MIST!");
...
static void Cmd_EscribeTextoSustituto(void)
{
    ...
    const u8 *texto = (TransitorioActivo(gBattlerAttacker, TRANSITORIO_SUSTITUTO))
                    ? COMPOUND_STRING("{B_ATK_NAME_WITH_PREFIX} made a SUBSTITUTE!")
                    : COMPOUND_STRING("But it does not have enough HP left to make a substitute!");
...
static void Cmd_EscribeTextoReserva(void)
{
    ...
    // Cmd_stockpile marca RESULTADO_MOVIMIENTO_FALLO cuando ya no se puede reservar más.
    const u8 *texto = (gCombate->resultadoAtaque == RESULTADO_MOVIMIENTO_FALLO)
                    ? COMPOUND_STRING("{B_ATK_NAME_WITH_PREFIX} can't stockpile any more!")
                    : COMPOUND_STRING("{B_ATK_NAME_WITH_PREFIX} stockpiled {B_BUFF1}!");
...
static void Cmd_EscribeTextoTragarFalla(void)
{
    ...
    const u8 *texto = (gBattleMons[gBattlerAttacker].hp == gBattleMons[gBattlerAttacker].maxHP)
                    ? COMPOUND_STRING("{B_DEF_NAME_WITH_PREFIX}'s HP is full!")
                    : COMPOUND_STRING("But it failed to swallow a thing!");

    gBattlescriptCurrInstr = cmd->nextInstr;
    EscribeTextoCombate(gBattlerAttacker, texto);
}

// Descanso pisa estado al dormir, así que el "además se curó" hay que anotarlo.
static void Cmd_EscribeTextoDescanso(void)
{
    CMD_ARGS();
    ...
    const u8 *texto = gDescansoCuroEstado
                    ? COMPOUND_STRING("{B_ATK_NAME_WITH_PREFIX} slept and became healthy!")
                    : COMPOUND_STRING("{B_ATK_NAME_WITH_PREFIX} went to sleep!");
...
static void Cmd_EscribeTextoAbsorbeFuego(void)
{
    ...
    const u8 *texto = gAbsorbeFuegoSubioPotencia
                    ? COMPOUND_STRING("{B_DEF_NAME_WITH_PREFIX}'s {B_DEF_ABILITY} raised its FIRE power!")
                    : COMPOUND_STRING("{B_DEF_NAME_WITH_PREFIX}'s {B_DEF_ABILITY} made {B_CURRENT_MOVE} ineffective!");
...
static void Cmd_EscribeTextoBaya(void)
{
    ...
    const u8 *texto = gBayaNormalizoEstado
                    ? COMPOUND_STRING("{B_SCR_ACTIVE_NAME_WITH_PREFIX}'s {B_LAST_ITEM} normalized its status!")
                    : COMPOUND_STRING("{B_SCR_ACTIVE_NAME_WITH_PREFIX}'s {B_LAST_ITEM} cured its {B_BUFF1} problem!");

    gBattlescriptCurrInstr = cmd->nextInstr;
    EscribeTextoCombate(gBattlerAttacker, texto);
}

// Drenaje de PS. La rama de Viscosidad ya la decide el script con jumpifability, y
// la habilidad sigue puesta al imprimir, así que basta con mirarla.
static void Cmd_EscribeTextoDrenaje(void)
{
    CMD_ARGS();
    ...
    const u8 *texto = (HabilidadCombatiente(gBattlerTarget) == ABILITY_LIQUID_OOZE)
                    ? COMPOUND_STRING("It sucked up the liquid ooze!")
                    : COMPOUND_STRING("¡{B_DEF_NAME_WITH_PREFIX} fue drenado!");

    gBattlescriptCurrInstr = cmd->nextInstr;
    EscribeTextoCombate(gBattlerAttacker, texto);
}

// Drenaje del turno de Drenadoras: aquí la Viscosidad es la del que tiene la semilla.
static void Cmd_EscribeTextoDrenadorasTurno(void)
{
    CMD_ARGS();
    ...
    const u8 *texto = (HabilidadCombatiente(gBattlerAttacker) == ABILITY_LIQUID_OOZE)
                    ? COMPOUND_STRING("It sucked up the liquid ooze!")
                    : COMPOUND_STRING("{B_ATK_NAME_WITH_PREFIX}'s health is sapped by Leech Seed!");
...
static void Cmd_EscribeTextoDrenadoras(void)
{
    ...
    switch (gResultadoDrenadoras)
    {
    case DRENADORAS_PUESTAS: texto = COMPOUND_STRING("{B_DEF_NAME_WITH_PREFIX} was seeded!");        break;
    case DRENADORAS_FALLO:   texto = COMPOUND_STRING("{B_DEF_NAME_WITH_PREFIX} avoided the attack!"); break;
    case DRENADORAS_INMUNE:  texto = COMPOUND_STRING("It doesn't affect {B_DEF_NAME_WITH_PREFIX}…");  break;
    }
...
static void Cmd_EscribeTextoCambioObjeto(void)
{
    ...
    switch (gResultadoCambioObjeto)
    {
    case OBJETO_TOMADO: texto = COMPOUND_STRING("{B_ATK_NAME_WITH_PREFIX} obtuvo {B_BUFF1}."); break;
    case OBJETO_DADO:   texto = COMPOUND_STRING("{B_DEF_NAME_WITH_PREFIX} obtuvo {B_BUFF2}."); break;
    case OBJETO_AMBOS:  texto = COMPOUND_STRING("{B_ATK_NAME_WITH_PREFIX} obtuvo {B_BUFF1}. {B_DEF_NAME_WITH_PREFIX} obtuvo {B_BUFF2}."); break;
    }
...
// Los seis mensajes de habilidad al entrar son de habilidades distintas, así que la
// propia habilidad los distingue.
static void Cmd_EscribeTextoHabilidadEntrada(void)
{
    CMD_ARGS();
    ...
    case ABILITY_MOLD_BREAKER:
        texto = COMPOUND_STRING("{B_SCR_ACTIVE_NAME_WITH_PREFIX} breaks the mold!");
        break;
    case ABILITY_UNNERVE:
        texto = COMPOUND_STRING("The opposing team is too nervous to eat Berries!");
        break;
    case ABILITY_GENERADOR:
        texto = COMPOUND_STRING("¡{B_SCR_ACTIVE_NAME_WITH_PREFIX} genera energía y potencia ataques eléctricos de su equipo!");
        break;
    case ABILITY_SCREEN_CLEANER:
        texto = COMPOUND_STRING("All screens on the field were cleansed!");
        break;
    case ABILITY_NEUTRALIZING_GAS:
        texto = COMPOUND_STRING("Neutralizing gas filled the area!");
        break;
    }
...
static void Cmd_EscribeTextoHierbaMental(void)
{
    ...
    switch (gCuraHierbaMental)
    {
    case HIERBA_ENAMORAMIENTO: texto = COMPOUND_STRING("{B_ATK_NAME_WITH_PREFIX} got over its infatuation!"); break;
    case HIERBA_MOFA:          texto = COMPOUND_STRING("{B_ATK_NAME_WITH_PREFIX}'s {B_BUFF1} wore off!");      break;
    case HIERBA_BIS:           texto = COMPOUND_STRING("{B_ATK_NAME_WITH_PREFIX}'s ENCORE ended!");            break;
    case HIERBA_TORMENTO:      texto = COMPOUND_STRING("{B_ATK_NAME_WITH_PREFIX} is no longer tormented!");    break;
    case HIERBA_ANULACION:     texto = COMPOUND_STRING("{B_ATK_NAME_WITH_PREFIX} is disabled no more!");       break;
    }
...
// Reflejo, Pantalla Luz y Velo Sagrado. Las variantes "single"/"double" de la tabla
// vieja tenían el mismo texto, así que sus seis entradas eran en realidad cuatro.
static void Cmd_EscribeTextoBarrera(void)
{
    CMD_ARGS();
    ...
    switch (gTipoBarreraPuesta)
    {
    case BARRERA_FALLO:        texto = COMPOUND_STRING("¡Pero falló!");                                       break;
    case BARRERA_REFLEJO:      texto = COMPOUND_STRING("{B_ATK_PREFIX2}'s {B_CURRENT_MOVE} raised DEFENSE!");  break;
    case BARRERA_PANTALLA_LUZ: texto = COMPOUND_STRING("{B_ATK_PREFIX2}'s {B_CURRENT_MOVE} raised SP. DEF!");  break;
    case BARRERA_VELO_SAGRADO: texto = COMPOUND_STRING("{B_ATK_PREFIX2}'s party is covered by a veil!");       break;
    }
...
// Cascabel Cura / Aromaterapia. Las cuatro variantes de Insonorizar que tenía la
// tabla decían todas lo mismo: lo que cambia es a quién cura, no el texto.
static void Cmd_EscribeTextoCuraEquipo(void)
{
    CMD_ARGS();
    ...
    const u8 *texto = gCuraEquipoPorAroma
                    ? COMPOUND_STRING("A soothing aroma wafted through the area!")
                    : COMPOUND_STRING("A bell chimed!");
...
static void Cmd_jumpifinsonorizadoatacante(void)
{
    ...
    if (gInsonorizadoCascabel & INSONORIZADO_ATACANTE)
        gBattlescriptCurrInstr = cmd->nextInstr;
    else
        gBattlescriptCurrInstr = cmd->jumpInstr;
}
...
static void Cmd_jumpifinsonorizadoaliado(void)
{
    ...
    if (gInsonorizadoCascabel & INSONORIZADO_ALIADO)
        gBattlescriptCurrInstr = cmd->nextInstr;
    else
        gBattlescriptCurrInstr = cmd->jumpInstr;
}

// El movimiento que atrapó ya está guardado, así que no hace falta el índice que
// calculaba el bucle sobre sTrappingMoves.
static void Cmd_EscribeTextoAtrapado(void)
{
    CMD_ARGS();
    ...
    switch (gCombate->wrappedMove[gEffectBattler])
    {
    case MOVE_BIND:
        texto = COMPOUND_STRING("{B_DEF_NAME_WITH_PREFIX} was squeezed by {B_ATK_NAME_WITH_PREFIX}'s BIND!");
        break;
    case MOVE_WRAP:
        texto = COMPOUND_STRING("{B_DEF_NAME_WITH_PREFIX} was WRAPPED by {B_ATK_NAME_WITH_PREFIX}!");
        break;
    case MOVE_CLAMP:
        texto = COMPOUND_STRING("{B_DEF_NAME_WITH_PREFIX} was CLAMPED by {B_ATK_NAME_WITH_PREFIX}!");
        break;
    case MOVE_SAND_TOMB:
        texto = COMPOUND_STRING("{B_DEF_NAME_WITH_PREFIX} was trapped by SAND TOMB!");
        break;
    case MOVE_MAGMA_STORM:
        texto = COMPOUND_STRING("{B_DEF_NAME_WITH_PREFIX} became trapped by swirling magma!");
        break;
    case MOVE_INFESTATION:
        texto = COMPOUND_STRING("{B_DEF_NAME_WITH_PREFIX} has been afflicted with an infestation by {B_ATK_NAME_WITH_PREFIX}!");
        break;
    case MOVE_THUNDER_CAGE:
        texto = COMPOUND_STRING("{B_ATK_NAME_WITH_PREFIX} trapped {B_DEF_NAME_WITH_PREFIX}!");
        break;
    // Fuego Fatuo y Torbellino comparten el mensaje del vórtice.
    default:
        texto = COMPOUND_STRING("{B_DEF_NAME_WITH_PREFIX} was trapped in the vortex!");
        break;
    }
...
static void Cmd_EscribeTextoDanioTrampa(void)
{
    ...
    static const u8 *const sTextosTrampa[NUMERO_TRAMPAS_DANIO] =
    {
        [TRAMPA_BRASAS]            = COMPOUND_STRING("¡{B_SCR_ACTIVE_NAME_WITH_PREFIX} se ha quemado con las brasas!"),
        [TRAMPA_ESPINAS]           = COMPOUND_STRING("¡{B_SCR_ACTIVE_NAME_WITH_PREFIX} se ha clavado las espinas!"),
        [TRAMPA_PUAS_TOXICAS]      = COMPOUND_STRING("¡{B_SCR_ACTIVE_NAME_WITH_PREFIX} se ha pinchado con las púas tóxicas!"),
        [TRAMPA_ROCAS_PUNTIAGUDAS] = COMPOUND_STRING("¡Las rocas puntiagudas han herido a {B_SCR_ACTIVE_NAME_WITH_PREFIX}!"),
        [TRAMPA_PUAS]              = COMPOUND_STRING("¡{B_SCR_ACTIVE_NAME_WITH_PREFIX} se ha pinchado con las púas!"),
        [TRAMPA_PENITENTES]        = COMPOUND_STRING("¡Los penitentes han herido a {B_SCR_ACTIVE_NAME_WITH_PREFIX}!"),
    };

    const u8 *texto = gTrampaEntradaActiva < NUMERO_TRAMPAS_DANIO
                    ? sTextosTrampa[gTrampaEntradaActiva]
                    : COMPOUND_STRING("¡{B_SCR_ACTIVE_NAME_WITH_PREFIX} ha pisado una trampa!");
...
static void Cmd_EscribeTextoBallEscapa(void)
{
    ...
    switch (gSacudidasBall)
    {
    case 0: texto = COMPOUND_STRING("Oh, no! The POKéMON broke free!"); break;
    case 1: texto = COMPOUND_STRING("Aww! It appeared to be caught!");  break;
    case 2: texto = COMPOUND_STRING("Aargh! Almost had it!");           break;
    default: break; // tres sacudidas y se escapa: sin mensaje propio
    }
...
// Sustituye a `setword gMensajeBatalla, B_MSG_ABILITY_PREVENTS_MOVE_STATUS`.
static void Cmd_MarcaPrevencionPorHabilidadPropia(void)
{
    CMD_ARGS();

    gResultadoPrevencionEstado = PREVENCION_POR_HABILIDAD_PROPIA;
    gBattlescriptCurrInstr = cmd->nextInstr;
}
...
void SetMoveEffect(bool32 primary)
{
    ...
    if (!primary && affectsUser != MOVE_EFFECT_AFFECTS_USER && !(gMarcasGolpe[MARCA_EFECTO_HABILIDAD_ESTADO]) && (battlerAbility == ABILITY_SHIELD_DUST || GetBattlerHoldEffect(gEffectBattler, TRUE) == HOLD_EFFECT_COVERT_CLOAK))
    {
    ...
    if (gSideStatuses[GetBattlerSide(gEffectBattler)] & SIDE_STATUS_SAFEGUARD && !(gMarcasGolpe[MARCA_EFECTO_HABILIDAD_ESTADO]) && !primary && gBattleScripting.moveEffect <= MOVE_EFFECT_CONFUSION)
        INCREMENT_RESET_RETURN

    if (!(gMarcasGolpe[MARCA_EFECTO_HABILIDAD_ESTADO]) && EstaPotenciadoPotenciaBruta(gBattlerAttacker, gCurrentMove) && !primary && gBattleScripting.moveEffect != MOVE_EFFECT_CHARGING)
        INCREMENT_RESET_RETURN
    ...
    if (gBattleScripting.moveEffect <= PRIMARY_STATUS_MOVE_EFFECT) // status change
    {
        ...
        case ESTADO_VENENO:
            if ((battlerAbility == ABILITY_IMMUNITY) && (primary == TRUE))
            {
                ...
                if (gMarcasGolpe[MARCA_EFECTO_HABILIDAD_ESTADO])
                {
                    gResultadoPrevencionEstado = PREVENCION_POR_HABILIDAD_ATACANTE;
            ...
            if (!CanPoisonType(gEffectBattler) && (gMarcasGolpe[MARCA_EFECTO_HABILIDAD_ESTADO]) && (primary == TRUE))
            {
                BattleScriptPush(gBattlescriptCurrInstr + 1);
                gBattlescriptCurrInstr = BattleScript_PSNPrevention;

                gResultadoPrevencionEstado = PREVENCION_SIN_EFECTO;
                RESET_RETURN
            }
        ...
        case ESTADO_QUEMADURA:
            ...
            if (EsTipo(gEffectBattler, TIPO_FUEGO) && (gMarcasGolpe[MARCA_EFECTO_HABILIDAD_ESTADO]) && (primary == TRUE))
            {
                BattleScriptPush(gBattlescriptCurrInstr + 1);
                gBattlescriptCurrInstr = BattleScript_BRNPrevention;
        ...
        case ESTADO_PARALISIS:
            ...
            if (!CanParalyzeType(gEffectBattler) && (gMarcasGolpe[MARCA_EFECTO_HABILIDAD_ESTADO]) && (primary == TRUE))
            {
                BattleScriptPush(gBattlescriptCurrInstr + 1);
                gBattlescriptCurrInstr = BattleScript_PRLZPrevention;

                gElegidorTextoMultiple = 2;
                RESET_RETURN
            }
        ...
        if (statusChanged == TRUE)
        {
            ...
            PonEstado(gEffectBattler, sStatusFlagsForMoveEffects[gBattleScripting.moveEffect]);

            gBattlescriptCurrInstr = sMoveEffectBS_Ptrs[gBattleScripting.moveEffect];

            ComandoFijaDatosPokemon(gEffectBattler, REQUEST_STATUS_BATTLE, 0, sizeof(gBattleMons[gEffectBattler].estado), &gBattleMons[gEffectBattler].estado);
            MarcaCombatienteOcupado(gEffectBattler);

            if (gMarcasGolpe[MARCA_EFECTO_HABILIDAD_ESTADO])
            {
                gEstadoAlteradoPorHabilidad = TRUE;
                gMarcasGolpe[MARCA_EFECTO_HABILIDAD_ESTADO] = FALSE;
            }
            else
            {
                gEstadoAlteradoPorHabilidad = FALSE;
            }
            ...
            if (gBattleScripting.moveEffect == MOVE_EFFECT_POISON || gBattleScripting.moveEffect == MOVE_EFFECT_TOXIC || gBattleScripting.moveEffect == MOVE_EFFECT_PARALYSIS || gBattleScripting.moveEffect == MOVE_EFFECT_BURN)
            {
                ...
                gMarcasGolpe[MARCA_EFECTO_SINCRONIA] = TRUE;
            }
            return;
        }
    ...
    {
        if (sTransitorioPorEfecto[gBattleScripting.moveEffect] != 0
         && TransitorioActivo(gEffectBattler, sTransitorioPorEfecto[gBattleScripting.moveEffect] - 1))
        {
            gBattlescriptCurrInstr++;
        }
        else
        {
            ...
            case MOVE_EFFECT_CONFUSION:
                ...
                {
                    PonTransitorio(gEffectBattler, TRANSITORIO_CONFUSION); // 2-5 turns
                    BattleScriptPush(gBattlescriptCurrInstr + 1);
                    gBattlescriptCurrInstr = sMoveEffectBS_Ptrs[gBattleScripting.moveEffect];
                }
                break;
            case MOVE_EFFECT_FLINCH:
                ...
                {
                    PonTransitorio(gEffectBattler, sTransitorioPorEfecto[gBattleScripting.moveEffect] - 1);
                    gBattlescriptCurrInstr++;
                }
                else
                {
            ...
            case MOVE_EFFECT_CHARGING:
                PonTransitorio(gEffectBattler, TRANSITORIO_MULTITURNO);
                gLockedMoves[gEffectBattler] = gCurrentMove;
                gProtectStructs[gEffectBattler].chargingTurn = TRUE;
                gBattlescriptCurrInstr++;
                break;
            case MOVE_EFFECT_WRAP:
                if (TransitorioActivo(gEffectBattler, TRANSITORIO_SIN_ESCAPE))
                {
                    gBattlescriptCurrInstr++;
                }
                else
                {
                    // Atrapar ya no cuenta turnos ni hace dano: solo impide huir,
                    // y dura hasta que quien lo hizo se vaya del campo.
                    PonTransitorio(gEffectBattler, TRANSITORIO_SIN_ESCAPE);
                    ...
                    // Aquí había un bucle sobre sTrappingMoves cuyo único fin era
                    // calcular el índice del mensaje. El movimiento ya queda en
                    // gCombate->wrappedMove, así que EscribeTextoAtrapado lo mira
                    // directamente.
                }
                break;
            ...
            case MOVE_EFFECT_RECHARGE:
                PonTransitorio(gEffectBattler, TRANSITORIO_DESCANSO);
                gDisableStructs[gEffectBattler].rechargeTimer = 2;
                gLockedMoves[gEffectBattler] = gCurrentMove;
                gBattlescriptCurrInstr++;
                break;
            case MOVE_EFFECT_RAGE:
                PonTransitorio(gBattlerAttacker, TRANSITORIO_FURIA);
                gBattlescriptCurrInstr++;
                break;
            case MOVE_EFFECT_STEAL_ITEM:
            {
            ...
            case MOVE_EFFECT_PREVENT_ESCAPE:
                PonTransitorio(gBattlerTarget, TRANSITORIO_SIN_ESCAPE);
            ...
            case MOVE_EFFECT_THRASH:
                // Se encadena al hueco del campo, no al Pokemon: si al que hay
                // ahi lo cambian, el siguiente que ocupe la posicion se lleva
                // los golpes igual.
                PonTransitorio(gEffectBattler, TRANSITORIO_MULTITURNO); PonTransitorio(gEffectBattler, TRANSITORIO_ENCADENADO);
                gLockedMoves[gEffectBattler] = gCurrentMove;
                gDisableStructs[gEffectBattler].posicionFijada = gBattlerTarget;
                break;
            ...
            case MOVE_EFFECT_FLAME_BURST:
                if (IsBattlerAlive(ALIADO(gBattlerTarget)) && !ES_SEMI_INVULNERABLE(ALIADO(gBattlerTarget)) && HabilidadCombatiente(ALIADO(gBattlerTarget)) != ABILITY_MAGIC_GUARD)
                {
            ...
            case MOVE_EFFECT_TRAP_BOTH:
                if (!(TransitorioActivo(gBattlerTarget, TRANSITORIO_SIN_ESCAPE)) && !(TransitorioActivo(gBattlerAttacker, TRANSITORIO_SIN_ESCAPE)))
                {
                    BattleScriptPush(gBattlescriptCurrInstr + 1);
                    gBattlescriptCurrInstr = BattleScript_BothCanNoLongerEscape;
                }
                if (!(TransitorioActivo(gBattlerTarget, TRANSITORIO_SIN_ESCAPE)))
                    gDisableStructs[gBattlerTarget].battlerPreventingEscape = gBattlerAttacker;

                if (!(TransitorioActivo(gBattlerAttacker, TRANSITORIO_SIN_ESCAPE)))
                    gDisableStructs[gBattlerAttacker].battlerPreventingEscape = gBattlerTarget;

                PonTransitorio(gBattlerTarget, TRANSITORIO_SIN_ESCAPE);
                PonTransitorio(gBattlerAttacker, TRANSITORIO_SIN_ESCAPE);
                break;
            case MOVE_EFFECT_DIRE_CLAW:
                break;
            case MOVE_EFFECT_STEALTH_ROCK:
                if (ColocaTrampaEntrada(TRAMPA_ROCAS_PUNTIAGUDAS, GetBattlerSide(gEffectBattler)))
                {
                    BattleScriptPush(gBattlescriptCurrInstr + 1);
                ...
                break;
            case MOVE_EFFECT_SPIKES:
                if (ColocaTrampaEntrada(TRAMPA_PUAS, GetBattlerSide(gEffectBattler)))
                {
                    BattleScriptPush(gBattlescriptCurrInstr + 1);
...
// Coloca una trampa en el lado del rival. Con TRAMPA_SEGUN_TIPO_MOVIMIENTO la
// trampa la decide el tipo del movimiento que se esta usando, que es lo que
// permite que Brasas, Espinas, Púas, Púas tóxicas, Rocas puntiagudas y
// Penitentes compartan un unico guion.
static void Cmd_settypedhazard(void)
{
    CMD_ARGS(u8 trampa, const u8 *failInstr);

    u32 trampa = cmd->trampa;
    u32 lado = OPONENTE(GetBattlerSide(gBattlerAttacker));

    if (trampa == TRAMPA_SEGUN_TIPO_MOVIMIENTO)
    {
        // Hay dos trampas de tipo Planta, Espinas y Enredaderas, asi que el tipo
        // solo no basta: el efecto del movimiento dice de cual de las dos
        // familias se trata.
        if (gMovimientos[gCurrentMove].effect == EFFECT_STICKY_WEB)
            trampa = (gMovimientos[gCurrentMove].type == TIPO_PLANTA) ? TRAMPA_ENREDADERAS : TRAMPA_RED_VISCOSA;
        else
            trampa = TrampaEntradaPorTipo(gMovimientos[gCurrentMove].type);
    }

    if (trampa >= NUMERO_TRAMPAS_ENTRADA || !ColocaTrampaEntrada(trampa, lado))
    {
        gBattlescriptCurrInstr = cmd->failInstr;
    }
    else
    {
        gTrampaEntradaActiva = trampa;
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
}

// Texto de "han caido X en el campo". Lo pide el guion despues de colocarla.
static void Cmd_EscribeTextoTrampaPuesta(void)
{
    CMD_ARGS();

    static const u8 *const sTextos[NUMERO_TRAMPAS_ENTRADA] =
    {
        [TRAMPA_BRASAS]            = COMPOUND_STRING("¡Han quedado brasas ardiendo en el campo de {B_DEF_TEAM2}!"),
        [TRAMPA_ESPINAS]           = COMPOUND_STRING("¡Han brotado espinas en el campo de {B_DEF_TEAM2}!"),
        [TRAMPA_PUAS_TOXICAS]      = COMPOUND_STRING("¡Han caído púas tóxicas en el campo de {B_DEF_TEAM2}!"),
        [TRAMPA_ROCAS_PUNTIAGUDAS] = COMPOUND_STRING("¡Flotan rocas puntiagudas sobre el campo de {B_DEF_TEAM2}!"),
        [TRAMPA_PUAS]              = COMPOUND_STRING("¡Han caído púas en el campo de {B_DEF_TEAM2}!"),
        [TRAMPA_PENITENTES]        = COMPOUND_STRING("¡Han crecido penitentes de hielo en el campo de {B_DEF_TEAM2}!"),
        [TRAMPA_RED_VISCOSA]       = COMPOUND_STRING("¡Una red viscosa cubre el campo de {B_DEF_TEAM2}!"),
        [TRAMPA_ENREDADERAS]       = COMPOUND_STRING("¡Han crecido enredaderas en el campo de {B_DEF_TEAM2}!"),
    };
    ...
    gBattlescriptCurrInstr = cmd->nextInstr;
    if (gTrampaEntradaActiva < NUMERO_TRAMPAS_ENTRADA)
        EscribeTextoCombate(gBattlerAttacker, sTextos[gTrampaEntradaActiva]);
}


// Las marcas del golpe ya no son bits dentro de una palabra, asi que orword y
// bicword no valen: cada una es su propia variable y se toca por su nombre.
static void Cmd_ponmarca(void)
{
    CMD_ARGS(u8 marca);

    gMarcasGolpe[cmd->marca] = TRUE;
    gBattlescriptCurrInstr = cmd->nextInstr;
}
...
static void Cmd_quitamarca(void)
{
    ...
    gMarcasGolpe[cmd->marca] = FALSE;
    gBattlescriptCurrInstr = cmd->nextInstr;
}

static void Cmd_jumpifmarca(void)
{
    CMD_ARGS(u8 marca, u8 valor, const u8 *jumpInstr);

    gBattlescriptCurrInstr = (gMarcasGolpe[cmd->marca] == cmd->valor) ? cmd->jumpInstr : cmd->nextInstr;
}
...
static void Cmd_clearstatusfromeffect(void)
{
    ...
    if (gBattleScripting.moveEffect <= PRIMARY_STATUS_MOVE_EFFECT)
        QuitaEstado(battler, sStatusFlagsForMoveEffects[gBattleScripting.moveEffect]);
    else
    {
        if (sTransitorioPorEfecto[gBattleScripting.moveEffect] != 0)
            QuitaTransitorio(battler, sTransitorioPorEfecto[gBattleScripting.moveEffect] - 1);
...
static void Cmd_tryfaintmon(void)
{
    ...
    if (cmd->isSpikes != 0)
    {
        if (gCombatienteDebilitado[battler])
        {
            BattleScriptPop();
    ...
    else
    {
        ...
        {
            gCombatienteDebilitado[battler] = TRUE;
            BattleScriptPush(cmd->nextInstr);
            gBattlescriptCurrInstr = faintScript;
            if (GetBattlerSide(battler) == LADO_JUGADOR)
            {
                gMarcasGolpe[MARCA_JUGADOR_DEBILITADO] = TRUE;
            ...
            if ((gMarcasGolpe[MARCA_LAZO_DESTINO]) && IsBattlerAlive(gBattlerAttacker))
            {
...
static void Cmd_cleareffectsonfaint(void)
{
    ...
    if (!HayAlgunCombatienteOcupado())
    {
        ...
        {
            QuitaTodosEstados(battler);
            ComandoFijaDatosPokemon(battler, REQUEST_STATUS_BATTLE, 0, sizeof(gBattleMons[battler].estado), &gBattleMons[battler].estado);
            MarcaCombatienteOcupado(battler);
        }
...
// Salta si el combatiente tiene ese estado principal. ESTADO_CUALQUIERA vale
// por todos, para los movimientos y objetos que curan lo que sea que haya.
static void Cmd_jumpifestado(void)
{
    CMD_ARGS(u8 battler, u8 estado, const u8 *jumpInstr);

    u32 battler = GetBattlerForBattleScript(cmd->battler);
    bool32 lo_tiene = (cmd->estado == ESTADO_CUALQUIERA)
                    ? HayAlgunEstado(battler)
                    : EstadoActivo(battler, cmd->estado);

    if (lo_tiene && IsBattlerAlive(battler))
        gBattlescriptCurrInstr = cmd->jumpInstr;
    else
        gBattlescriptCurrInstr = cmd->nextInstr;
}

// Salta segun tenga o no un estado transitorio. Con negado a TRUE salta
// cuando NO lo tiene.
static void Cmd_jumpiftransitorio(void)
{
    CMD_ARGS(u8 battler, u8 transitorio, bool8 negado, const u8 *jumpInstr);

    u32 battler = GetBattlerForBattleScript(cmd->battler);
    bool32 activo = TransitorioActivo(battler, cmd->transitorio) && IsBattlerAlive(battler);

    if (activo != (cmd->negado != 0))
        gBattlescriptCurrInstr = cmd->jumpInstr;
    else
        gBattlescriptCurrInstr = cmd->nextInstr;
}
        ...
        if (!HayAlgunCombatienteOcupado())
        {
            ...
            else
            {
                ...
                if (IsValidForBattle(&gPlayerParty[*expMonId]))
                {
                    ...
                    if (wasSentOut)
                        EscribeTextoCombate(gCombate->expGetterBattlerId, COMPOUND_STRING("¡{B_BUFF1} ganó {B_BUFF2} {B_BUFF3} puntos de experiencia!"));
...
static void Cmd_checkteamslost(void)
{
    ...
    if (gBattleOutcome == 0 && (EsCombateContraEntrenador(gCombate->tipoCombate)))
    {
        ...
        {
            if ((gCombatienteDebilitado[i]) && (!gSpecialStatuses[i].faintedHasReplacement))
                emptyPlayerSpots++;
        }
...
static void PlayAnimation(u32 battler, u8 animId, const u16 *argPtr, const u8 *nextInstr)
{
    ...
    else if (gMarcasGolpe[MARCA_SIN_ANIMACION] && animId != B_ANIM_RESTORE_BG)
    {
    ...
    else if (ES_SEMI_INVULNERABLE(battler))
    {
        gBattlescriptCurrInstr = nextInstr;
    }
    else
...
static void Cmd_moveend(void)
{
    ...
    do
    {
        ...
        case MOVEEND_RAGE: // rage check
            ...
            break;
        case MOVEEND_RECOIL:
            if (gMarcasGolpe[MARCA_NO_PUEDE_MOVERSE])
            {
                gBattleScripting.moveendState++;
                break;
            }
        ...
        case MOVEEND_CHOICE_MOVE: // update choice band move
        {
            ...
            {
                if ((gMovimientos[gChosenMove].effect == EFECTO_RELEVO) && gCombate->resultadoAtaque != RESULTADO_MOVIMIENTO_FALLIDO)
                {
                    gBattleScripting.moveendState++;
                    break;
                }
            ...
            {
                if (gBattleMons[gBattlerAttacker].movimientos[i] == *choicedMoveAtk)
                    break;
            }
        ...
        case MOVEEND_ATTACKER_INVISIBLE: // make attacker sprite invisible
            if (ES_SEMI_INVULNERABLE(gBattlerAttacker) && gMarcasGolpe[MARCA_SIN_ANIMACION])
            {
        ...
        case MOVEEND_ATTACKER_VISIBLE: // make attacker sprite visible
            if (!(MovimientoEsEfectivo(gCombate->resultadoMovimiento)) || !(ES_SEMI_INVULNERABLE(gBattlerAttacker)) || WasUnableToUseMove(gBattlerAttacker))
            {
                ComandoVisibilidadSprite(gBattlerAttacker, FALSE);
                MarcaCombatienteOcupado(gBattlerAttacker);
                QuitaTransitorio(gBattlerAttacker, TRANSITORIO_EN_EL_AIRE); QuitaTransitorio(gBattlerAttacker, TRANSITORIO_BAJO_TIERRA); QuitaTransitorio(gBattlerAttacker, TRANSITORIO_BAJO_EL_AGUA); QuitaTransitorio(gBattlerAttacker, TRANSITORIO_GOLPE_FANTASMA);
        ...
        case MOVEEND_TARGET_VISIBLE: // make target sprite visible
            if (!gSpecialStatuses[gBattlerTarget].restoredBattlerSprite && gBattlerTarget < gBattlersCount && !ES_SEMI_INVULNERABLE(gBattlerTarget))
            {
                ComandoVisibilidadSprite(gBattlerTarget, FALSE);
                MarcaCombatienteOcupado(gBattlerTarget);
                QuitaTransitorio(gBattlerTarget, TRANSITORIO_EN_EL_AIRE); QuitaTransitorio(gBattlerTarget, TRANSITORIO_BAJO_TIERRA); QuitaTransitorio(gBattlerTarget, TRANSITORIO_BAJO_EL_AGUA); QuitaTransitorio(gBattlerTarget, TRANSITORIO_GOLPE_FANTASMA);
                gBattleScripting.moveendState++;
                return;
            }
            gBattleScripting.moveendState++;
        ...
        case MOVEEND_UPDATE_LAST_MOVES:
            if (gMarcasGolpe[MARCA_INTERCAMBIA_ATACANTE])
            {
                u8 temp;
                SWAP(gBattlerAttacker, gBattlerTarget, temp);
                gMarcasGolpe[MARCA_INTERCAMBIA_ATACANTE] = FALSE;
            }
            gDisableStructs[gBattlerAttacker].usedMoves |= 1u << gCurrMovePos;
            if (gMarcasGolpe[MARCA_TEXTO_ATAQUE_ESCRITO])
            {
            ...
            if (!(gAbsentBattlerFlags & (1u << gBattlerAttacker)) && !(gCombate->absentBattlerFlags & (1u << gBattlerAttacker)) && gMovimientos[originallyUsedMove].effect != EFECTO_RELEVO)
            {
                ...
                if (!(gCombatienteDebilitado[gBattlerTarget]))
        ...
        case MOVEEND_NEXT_TARGET: // For moves hitting two opposing Pokemon.
        {
            ...
            if (!(gMarcasGolpe[MARCA_NO_PUEDE_MOVERSE]) && EsCombateContraEntrenador(gCombate->tipoCombate) && !gProtectStructs[gBattlerAttacker].chargingTurn && (moveTarget == MOVE_TARGET_BOTH || moveTarget == MOVE_TARGET_FOES_AND_ALLY) && !(gMarcasGolpe[MARCA_SIN_TEXTO_ATAQUE]))
            {
                ...
                gMarcasGolpe[MARCA_SIN_TEXTO_ATAQUE] = TRUE;
                gMarcasGolpe[MARCA_SIN_GASTAR_PP] = FALSE;
            }
        ...
        {
            if (MovimientoEsEfectivo(gCombate->resultadoMovimiento) && !(gMarcasGolpe[MARCA_NO_PUEDE_MOVERSE]) && gCombate->contadorMultigolpes && !(gMovimientos[gCurrentMove].effect == EFFECT_PRESENT && gCombate->presentBasePower == 0)) // Silly edge case
            {
                ...
                else
                {
                    ...
                    if (gBattleMons[gBattlerAttacker].hp && gBattleMons[gBattlerTarget].hp
                        && (gChosenMove == MOVE_SNORE || !(EstadoActivo(gBattlerAttacker, ESTADO_SUENO)))
                        && !(EstadoActivo(gBattlerAttacker, ESTADO_CONGELACION)))
                    {
        ...
        case MOVEEND_CLEAR_BITS:                                                                                   // Clear/Set bits for things like using a move for all targets and all hits.
            if (B_RAMPAGE_CANCELLING >= GEN_5 && MoveHasAdditionalEffectSelf(gCurrentMove, MOVE_EFFECT_THRASH)     // If we're rampaging
                && !(MovimientoEsEfectivo(gCombate->resultadoMovimiento))                                          // And it is unusable
                && (TransitorioActivo(gBattlerAttacker, TRANSITORIO_ENCADENADO)))                                   // And is still chained
            ...
            if (B_CHARGE <= GEN_8 || moveType == TIPO_ELECTRICO)
                QuitaTransitorio(gBattlerAttacker, TRANSITORIO_CARGADO);
            gBattleScripting.moveendState++;
            break;
        case MOVEEND_COUNT:
            break;
...
static void Cmd_returnatktoball(void)
{
    ...
    if (!(gCombatienteDebilitado[gBattlerAttacker]))
    {
...
static void Cmd_openpartyscreen(void)
{
    ...
    if (cmd->battler == BS_FAINTED_MULTIPLE_1)
    {
        if (!EsCombateContraEntrenador(gCombate->tipoCombate))
        {
            for (battler = 0; battler < gBattlersCount; battler++)
            {
                if (gCombatienteDebilitado[battler])
                {
                    ...
                    {
                        gAbsentBattlerFlags |= 1u << battler;
                        gCombatienteDebilitado[battler] = FALSE;
                        MarcaCombatienteOcupado(battler);
                    }
        ...
        else if (EsCombateContraEntrenador(gCombate->tipoCombate))
        {
            ...
            for (i = 0; i < gBattlersCount; i++)
            {
                ...
                {
                    if (i > 1 && gCombatienteDebilitado[ALIADO(i)])
                        continue;
            ...
            for (i = 0; i < NUMERO_LADOS; i++)
            {
                if (!(gSpecialStatuses[i].faintedHasReplacement))
                {
                    ...
                    if (!hasReplacement && HayAlgunCombatienteDebilitado())
                    {
    ...
    else if (cmd->battler == BS_FAINTED_MULTIPLE_2)
    {
        if (EsCombateContraEntrenador(gCombate->tipoCombate))
        {
            for (i = 0; i < NUMERO_LADOS; i++)
            {
                if (gCombatienteDebilitado[ALIADO(i)] && gCombatienteDebilitado[i])
                {
                    battler = ALIADO(i);
                    if (HasNoMonsToSwitch(battler, PARTY_SIZE, PARTY_SIZE))
                    {
        ...
        gBattlerFainted = 0;
        while (!gCombatienteDebilitado[gBattlerFainted] && gBattlerFainted < gBattlersCount)
            gBattlerFainted++;
    ...
    else
    {
        if (cmd->partyScreenOptional)
            accionEquipo = PARTY_ACTION_CHOOSE_MON;
        else
            accionEquipo = PARTY_ACTION_SEND_OUT;
        ...
        else
        {
            ...
            *(gCombate->monToSwitchIntoId + battler) = PARTY_SIZE;
            ...
            ComandoEligePokemon(battler, accionEquipo, *(gCombate->monToSwitchIntoId + ALIADO(battler)), ABILITY_NONE, gCombate->battlerPartyOrders[battler]);
            MarcaCombatienteOcupado(battler);
...
static void SetDmgHazardsBattlescript(u32 battler, u32 trampa)
{
    gMarcasGolpe[MARCA_LAZO_DESTINO] = FALSE;
    gBattleScripting.battler = battler;
    gTrampaEntradaActiva = trampa;
...
bool32 DoSwitchInAbilities(u32 battler)
{
    return (AbilityBattleEffects(ABILITYEFFECT_ON_SWITCHIN, battler, 0, 0, 0) ||
           (ObtenClimaCombate() != CLIMA_COMBATE_NINGUNO && ClimaTieneEfecto() && AbilityBattleEffects(ABILITYEFFECT_ON_WEATHER, battler, 0, 0, 0)));
}
...
// Resuelve la siguiente trampa pendiente del lado en el que acaba de entrar el
// Pokemon y deja en marcha el guion que le corresponda. Devuelve TRUE mientras
// quede algo por hacer: quien llama vuelve a entrar hasta que no queda nada.
//
// El orden importa. Primero la absorcion, porque el que se lleva las trampas
// puestas no puede cobrar por ellas; luego el dano, una trampa por vez para que
// cada una tenga su mensaje; y al final las que bajan Velocidad.
static bool32 ProcesaTrampasEntrada(u32 battler)
{
    u32 lado = GetBattlerSide(battler);
    u16 *hechas = &gDisableStructs[battler].trampasEntradaProcesadas;

    // Volar, levitar o calzar botas libra de todas por igual.
    if (EsInmuneATrampas(battler))
        return FALSE;

    if (!(*hechas & TRAMPAS_ABSORCION_PROCESADA))
    {
        *hechas |= TRAMPAS_ABSORCION_PROCESADA;

        if (AbsorbeTrampasPorTipo(lado, gBattleMons[battler].types[TIPO_1],
                                        gBattleMons[battler].types[TIPO_2]) != 0)
        {
            gBattleScripting.battler = battler;
            BattleScriptPushCursor();
            gBattlescriptCurrInstr = BattleScript_TrampasAbsorbidas;
            return TRUE;
        }
    }
    ...
    {
        if ((*hechas & (1u << trampa)) || !HayTrampaEntrada(trampa, lado))
            continue;

        *hechas |= 1u << trampa;
        gBattleMoveDamage = DanioTrampa(TipoTrampaEntrada(trampa), battler);

        // Si resiste o es inmune al tipo de la trampa no se lleva nada, ni
        // siquiera un PS: se pasa a la siguiente.
        if (gBattleMoveDamage != 0)
        {
            SetDmgHazardsBattlescript(battler, trampa);
            return TRUE;
        }
    }

    for (u32 trampa = NUMERO_TRAMPAS_DANIO; trampa < NUMERO_TRAMPAS_ENTRADA; trampa++)
    {
        if ((*hechas & (1u << trampa)) || !HayTrampaEntrada(trampa, lado))
            continue;
...
static bool32 DoSwitchInEffectsForBattler(u32 battler)
{
    ...
    else if (ProcesaTrampasEntrada(battler))
    {
        // El guion que toque ya esta puesto; se vuelve a entrar hasta agotarlas.
    }
    else
    {
        ...
        gDisableStructs[battler].trampasEntradaProcesadas = 0;
...
static void Cmd_switchineffects(void)
{
    ...
    case BS_FAINTED_MULTIPLE_1: // Saves the battlers.
        ...
        do
        {
            ...
            if (gBattlerFainted >= gBattlersCount)
                break;
            if (gCombatienteDebilitado[gBattlerFainted] && !(gAbsentBattlerFlags & (1u << gBattlerFainted)))
                break;
        } while (1);
...
static void Cmd_hitanimation(void)
{
    ...
    else if (!(gMarcasGolpe[MARCA_IGNORA_SUSTITUTO]) || !(DoesSubstituteBlockMove(gBattlerAttacker, battler, gCurrentMove)) || gDisableStructs[battler].substituteHP == 0)
    {
...
static void Cmd_swapattackerwithtarget(void)
{
    ...
    if (gMarcasGolpe[MARCA_INTERCAMBIA_ATACANTE])
        ...
        gMarcasGolpe[MARCA_INTERCAMBIA_ATACANTE] = TRUE;
...
static void Cmd_statusanimation(void)
{
    ...
    {
        u32 battler = GetBattlerForBattleScript(cmd->battler);
        if (!ES_SEMI_INVULNERABLE(battler) && gDisableStructs[battler].substituteHP == 0 && !(gMarcasGolpe[MARCA_SIN_ANIMACION]))
        {
            ComandoAnimacionEstado(battler, FALSE, gBattleMons[battler].estado);
            MarcaCombatienteOcupado(battler);
        }
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
...
static void Cmd_animatransitorio(void)
{
    CMD_ARGS(u8 battler, u8 transitorio);

    if (!HayAlgunCombatienteOcupado())
    {
        u32 battler = GetBattlerForBattleScript(cmd->battler);
        u32 transitorio = cmd->transitorio;
        if (!ES_SEMI_INVULNERABLE(battler) && gDisableStructs[battler].substituteHP == 0 && !(gMarcasGolpe[MARCA_SIN_ANIMACION]))
        {
            ComandoAnimacionEstado(battler, TRUE, transitorio);
            MarcaCombatienteOcupado(battler);
        }
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
...
static bool32 HasAttackerFaintedTarget(void)
{
    if (MovimientoEsEfectivo(gCombate->resultadoMovimiento)
     ...
     && (gChosenMove == gMovimientoElegido[gBattlerAttacker] || gChosenMove == gBattleMons[gBattlerAttacker].movimientos[gChosenMovePos]))
        return TRUE;
    else
        return FALSE;
}
...
static bool32 TryDefogClear(u32 battlerAtk, bool32 clear)
{
    ...
    for (lado = 0; lado < NUMERO_LADOS; lado++)
    {
        ...
        gBattlerAttacker = lado; // For correct battle string. Ally's / Foe's
        if (HayAlgunaTrampaEntrada(lado))
        {
            QuitaTodasTrampasEntrada(lado);
            BattleScriptPushCursor();
            gBattlescriptCurrInstr = BattleScript_TrampasBarridas;
            return TRUE;
        }
    }
...
static void Cmd_various(void)
{
    ...
    case VARIOUS_INFATUATE_WITH_BATTLER:
    {
        ...
        gBattleScripting.battler = battler;
        PonEstado(battler, ESTADO_ENAMORADO);
        gBattlescriptCurrInstr = cmd->nextInstr;
        return;
    }
    ...
    case VARIOUS_GET_BATTLER_FAINTED:
    {
        VARIOUS_ARGS();
        if (gCombatienteDebilitado[battler])
            gEstadoMultiuso = TRUE;
        else
            gEstadoMultiuso = FALSE;
        break;
    }
    case VARIOUS_UPDATE_CHOICE_MOVE_ON_LVL_UP:
    {
        ...
        if (gBattlerPartyIndexes[0] == gCombate->expGetterMonId || gBattlerPartyIndexes[2] == gCombate->expGetterMonId)
        {
            ...
            for (i = 0; i < MAXIMO_MOVIMIENTOS_POKEMON; i++)
            {
                if (gBattleMons[battler].movimientos[i] == gCombate->choicedMove[battler])
                    break;
            }
            if (i == MAXIMO_MOVIMIENTOS_POKEMON)
    ...
    {
        VARIOUS_ARGS();
        PREPARE_STAT_BUFFER(gBattleTextBuff1, gEstadoMultiuso);
        break;
    }
    ...
    case VARIOUS_GOLPE_BAJO_CHECK:
    {
        ...
        if (GetBattlerTurnOrderNum(gBattlerAttacker) > GetBattlerTurnOrderNum(gBattlerTarget))
            gBattlescriptCurrInstr = cmd->failInstr;
        else if (EsMovimientoEstado(gBattleMons[gBattlerTarget].movimientos[gCombate->chosenMovePositions[gBattlerTarget]]))
            gBattlescriptCurrInstr = cmd->failInstr;
        else
            gBattlescriptCurrInstr = cmd->nextInstr;
        return;
    ...
    {
        VARIOUS_ARGS();
        if (gSideStatuses[GetBattlerSide(battler)] & SIDE_STATUS_AURORA_VEIL || !(ClimaTieneEfecto() && EsClimaCombateNieve(ObtenClimaCombate())))
        {
            gCombate->resultadoAtaque = RESULTADO_MOVIMIENTO_FALLO;
    ...
    case VARIOUS_JUMP_IF_TEAM_HEALTHY:
    {
        ...
        if ((EsCombateContraEntrenador(gCombate->tipoCombate)) && IsBattlerAlive(ALIADO(battler)))
        {
            ...
            if ((gBattleMons[battler].hp == gBattleMons[battler].maxHP && !(HayAlgunEstado(battler))) && (gBattleMons[partner].hp == gBattleMons[partner].maxHP && !(HayAlgunEstado(partner))))
                gBattlescriptCurrInstr = cmd->jumpInstr;
            else
                gBattlescriptCurrInstr = cmd->nextInstr;
        }
        ...
        {
            if (gBattleMons[battler].hp == gBattleMons[battler].maxHP && !(HayAlgunEstado(battler)))
                gBattlescriptCurrInstr = cmd->jumpInstr;
            else
                gBattlescriptCurrInstr = cmd->nextInstr;
        }
    ...
    case VARIOUS_CURE_CERTAIN_STATUSES:
    {
        ...
        {
            QuitaEstado(battler, ESTADO_ENAMORADO);
        ...
        // Check torment
        if (TransitorioActivo(battler, TRANSITORIO_TORMENTO))
        {
            QuitaTransitorio(battler, TRANSITORIO_TORMENTO);
...
static void Cmd_ColocaProteccion(void)
{
    ...
    if (fail)
    {
        ...
        gCombate->resultadoAtaque = RESULTADO_MOVIMIENTO_FALLO;
        gCombate->textoResultadoOverride = COMPOUND_STRING("¡Pero falló!");
    }
...
static void Cmd_setfieldweather(void)
{
    ...
    if (!IntentaCambiarClimaCombate(gBattlerAttacker, weather, ORIGEN_CLIMA_MOVIMIENTO))
    {
        gCombate->resultadoAtaque = RESULTADO_MOVIMIENTO_FALLO;
        gBattlescriptCurrInstr = cmd->nextInstr;
        return;
...
static void Cmd_setreflect(void)
{
    ...
    {
        gCombate->resultadoAtaque = RESULTADO_MOVIMIENTO_FALLO;
        gTipoBarreraPuesta = BARRERA_FALLO;
    }
    else
    {
        ...
        if (EsCombateContraEntrenador(gCombate->tipoCombate) && CountAliveMonsInBattle(BATTLE_ALIVE_SIDE, gBattlerAttacker) == 2)
            gTipoBarreraPuesta = BARRERA_REFLEJO;
        else
            gTipoBarreraPuesta = BARRERA_REFLEJO;
    }
    gBattlescriptCurrInstr = cmd->nextInstr;
...
static void Cmd_setseeded(void)
{
    ...
    if (!(MovimientoEsEfectivo(gCombate->resultadoMovimiento)) || TransitorioActivo(gBattlerTarget, TRANSITORIO_DRENADORAS))
    {
        gCombate->resultadoAtaque = RESULTADO_MOVIMIENTO_FALLO;
        gResultadoDrenadoras = DRENADORAS_FALLO;
    }
    else if (EsTipo(gBattlerTarget, TIPO_PLANTA))
    {
        gCombate->resultadoAtaque = RESULTADO_MOVIMIENTO_FALLO;
        gResultadoDrenadoras = DRENADORAS_INMUNE;
    }
    else
    {
        PonTransitorio(gBattlerTarget, TRANSITORIO_DRENADORAS);
        gResultadoDrenadoras = DRENADORAS_PUESTAS;
    }
...
static void Cmd_trysetrest(void)
{
    ...
    {
        if (HayAlgunEstado(gBattlerTarget) && !EstadoActivo(gBattlerTarget, ESTADO_SUENO))
            gDescansoCuroEstado = TRUE;
        else
            gDescansoCuroEstado = FALSE;

        PonEstado(gBattlerTarget, ESTADO_SUENO);
        ComandoFijaDatosPokemon(gBattlerTarget, REQUEST_STATUS_BATTLE, 0, sizeof(gBattleMons[gBattlerTarget].estado), &gBattleMons[gBattlerTarget].estado);
        MarcaCombatienteOcupado(gBattlerTarget);
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
}
...
static u32 ChangeStatBuffs(s8 statValue, u32 statId, u32 flags, const u8 *BS_ptr)
{
    ...
    u32 battler, battlerAbility, battlerHoldEffect;
    ...
    if (statValue <= -1) // Stat decrease.
    {
        ...
        else // try to decrease
        {
            ...
            if (gBattleMons[battler].statStages[statId] == 1)
                ...
                statValue = -2;
            if (statValue == -2)
                PREPARE_STRING_BUFFER(gBattleTextBuff2, COMPOUND_STRING("harshly {B_BUFF1} fell!"))
            else if (statValue <= -3)
                PREPARE_STRING_BUFFER(gBattleTextBuff2, COMPOUND_STRING("severely {B_BUFF1} fell!"))
            else
    ...
    else // stat increase
    {
        ...
        if (gBattleMons[battler].statStages[statId] == 11)
            ...
            statValue = 2;
        if (statValue == 2)
            PREPARE_STRING_BUFFER(gBattleTextBuff2, COMPOUND_STRING("sharply {B_BUFF1} rose!"))
        else if (statValue >= 3)
            PREPARE_STRING_BUFFER(gBattleTextBuff2, COMPOUND_STRING("drastically {B_BUFF1} rose!"))
        else
    ...
    if (gResultadoCambioEstadistica == CAMBIO_ESTADISTICA_TOPE && flags & STAT_CHANGE_ALLOW_PTR)
        gCombate->resultadoAtaque = RESULTADO_MOVIMIENTO_FALLO;
...
static void Cmd_setlightscreen(void)
{
    ...
    else
    {
        ...
        if (EsCombateContraEntrenador(gCombate->tipoCombate) && CountAliveMonsInBattle(BATTLE_ALIVE_SIDE, gBattlerAttacker) == 2)
            gTipoBarreraPuesta = BARRERA_PANTALLA_LUZ;
        else
            gTipoBarreraPuesta = BARRERA_PANTALLA_LUZ;
    }
...
static void Cmd_tryinfatuating(void)
{
    ...
    {
        if (!PuedeEnamorarse(gBattlerTarget, HabilidadCombatiente(gBattlerTarget)))
        {
            gBattlescriptCurrInstr = cmd->failInstr;
        }
        else
        {
            PonEstado(gBattlerTarget, ESTADO_ENAMORADO);
            gBattlescriptCurrInstr = cmd->nextInstr;
        }
    }
}
...
static void Cmd_updatestatusicon(void)
{
    ...
    if (cmd->battler == BS_ATTACKER_WITH_PARTNER)
    {
        ...
        {
            ComandoActualizaIconoEstado(battler);
            MarcaCombatienteOcupado(battler);
        }
        if ((EsCombateContraEntrenador(gCombate->tipoCombate)))
        {
...
static void Cmd_setmist(void)
{
    ...
    {
        gCombate->resultadoAtaque = RESULTADO_MOVIMIENTO_FALLIDO;
    }
    else
    {
...
static void Cmd_setsubstitute(void)
{
    ...
    else
    {
        ...
        PonTransitorio(gBattlerAttacker, TRANSITORIO_SUSTITUTO);
        QuitaTransitorio(gBattlerAttacker, TRANSITORIO_SIN_ESCAPE);
        gDisableStructs[gBattlerAttacker].substituteHP = gBattleMoveDamage;

        gMarcasGolpe[MARCA_IGNORA_SUSTITUTO] = TRUE;
    }
...
static void Cmd_disablelastusedattack(void)
{
    ...
    for (i = 0; i < MAXIMO_MOVIMIENTOS_POKEMON; i++)
    {
        if (gBattleMons[gBattlerTarget].movimientos[i] == gLastMoves[gBattlerTarget])
            break;
    }
    if (gDisableStructs[gBattlerTarget].disabledMove == MOVE_NONE && i != MAXIMO_MOVIMIENTOS_POKEMON && gBattleMons[gBattlerTarget].pp[i] != 0)
    {
        PREPARE_MOVE_BUFFER(gBattleTextBuff1, gBattleMons[gBattlerTarget].movimientos[i])

        gDisableStructs[gBattlerTarget].disabledMove = gBattleMons[gBattlerTarget].movimientos[i];
...
static void Cmd_trysetencore(void)
{
    ...
    {
        gDisableStructs[gBattlerTarget].encoredMove = gBattleMons[gBattlerTarget].movimientos[i];
...
static void Cmd_healpartystatus(void)
{
    ...
    if (gCurrentMove == MOVE_HEAL_BELL)
    {
        ...
        if (IsBattlerAlive(partner))
        {
            ...
            {
                QuitaTodosEstados(partner);
            }
            else
            {
                RecuerdaHabilidad(partner, gBattleMons[partner].ability);
                gInsonorizadoCascabel |= INSONORIZADO_ALIADO;
            }
        }
...
static void Cmd_cursetarget(void)
{
    ...
    if (TransitorioActivo(gBattlerTarget, TRANSITORIO_MALDICION))
    {
        gBattlescriptCurrInstr = cmd->failInstr;
    }
    else
    {
        // Maldicion ya no le cuesta PS a quien la lanza: es un movimiento de
        // estado y nada mas, como Fuego Fatuo lo es de la quemadura.
        PonTransitorio(gBattlerTarget, TRANSITORIO_MALDICION);
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
}
...
static void Cmd_trysetperishsong(void)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        if (TransitorioActivo(i, TRANSITORIO_CANTO_MORTAL) || HabilidadCombatiente(i) == ABILITY_SOUNDPROOF || BlocksPrankster(gCurrentMove, gBattlerAttacker, i, TRUE))
        {
            notAffectedCount++;
        }
        else
        {
            PonTransitorio(i, TRANSITORIO_CANTO_MORTAL);
...
static void Cmd_handlerollout(void)
{
    ...
    {
        if (!(TransitorioActivo(gBattlerAttacker, TRANSITORIO_MULTITURNO))) // First hit.
        {
            gDisableStructs[gBattlerAttacker].rolloutTimer = 5;
            gDisableStructs[gBattlerAttacker].rolloutTimerStartValue = 5;
            PonTransitorio(gBattlerAttacker, TRANSITORIO_MULTITURNO);
            gLockedMoves[gBattlerAttacker] = gCurrentMove;
        }
        if (--gDisableStructs[gBattlerAttacker].rolloutTimer == 0) // Last hit.
        {
            QuitaTransitorio(gBattlerAttacker, TRANSITORIO_MULTITURNO);
        }
...
static void Cmd_jumpifconfusedandstatmaxed(void)
{
    ...
    if (TransitorioActivo(gBattlerTarget, TRANSITORIO_CONFUSION) && !CompareStat(gBattlerTarget, cmd->stat, ESTADISTICA_MAS_6, COMPARACION_MENOR))
...
static void Cmd_setsafeguard(void)
{
    ...
    else
    {
        ...
        gTipoBarreraPuesta = BARRERA_VELO_SAGRADO;
    }
...
static void Cmd_jumpifnopursuitswitchdmg(void)
{
    ...
    if (gAccionElegida[gBattlerTarget] == B_ACTION_USE_MOVE
     ...
     && !((EstadoActivo(gBattlerTarget, ESTADO_SUENO) || EstadoActivo(gBattlerTarget, ESTADO_CONGELACION)))
...
static void Cmd_girorapidofree(void)
{
    ...
    else if (TransitorioActivo(gBattlerAttacker, TRANSITORIO_DRENADORAS))
    {
        QuitaTransitorio(gBattlerAttacker, TRANSITORIO_DRENADORAS);
        BattleScriptPushCursor();
        gBattlescriptCurrInstr = BattleScript_LeechSeedFree;
    }
    else if (HayAlgunaTrampaEntrada(atkSide))
    {
        // Un solo barrido: el giro se lleva todas las trampas del lado propio.
        QuitaTodasTrampasEntrada(atkSide);
        BattleScriptPushCursor();
        gBattlescriptCurrInstr = BattleScript_TrampasBarridas;
    }
    else
...
static void Cmd_setdefensecurlbit(void)
{
    ...
    PonTransitorio(gBattlerAttacker, TRANSITORIO_RIZO_DEFENSA);
    gBattlescriptCurrInstr = cmd->nextInstr;
}
...
static void Cmd_RecuperaSaludSegunClima(void)
{
    ...
    if (gBattleMons[gBattlerAttacker].hp != gBattleMons[gBattlerAttacker].maxHP)
    {
        ...
        {
            if (climaCombate == CLIMA_COMBATE_NINGUNO || !ClimaTieneEfecto())
                gBattleMoveDamage = CuantosPSMaximos(gBattlerAttacker) / 2;
...
static void Cmd_setsemiinvulnerablebit(void)
{
    ...
    if (gBattleMoveEffects[gMovimientos[gCurrentMove].effect].semiInvulnerableEffect == TRUE)
    {
        ...
        if (cmd->clear)
            ...
            PonTransitorio(gBattlerAttacker, transitorio);
    }
...
static void Cmd_setcharge(void)
{
    ...
    u8 battler = GetBattlerForBattleScript(cmd->battler);
    PonTransitorio(battler, TRANSITORIO_CARGADO);
    if (B_CHARGE < GEN_9)
...
static void Cmd_cureifburnedparalysedorpoisoned(void)
{
    ...
    if ((EstadoActivo(gBattlerAttacker, ESTADO_VENENO) || EstadoActivo(gBattlerAttacker, ESTADO_QUEMADURA) || EstadoActivo(gBattlerAttacker, ESTADO_PARALISIS) || EstadoActivo(gBattlerAttacker, ESTADO_CONGELACION)))
    {
        QuitaTodosEstados(gBattlerAttacker);
        gBattlescriptCurrInstr = cmd->nextInstr;
        ComandoFijaDatosPokemon(gBattlerAttacker, REQUEST_STATUS_BATTLE, 0, sizeof(gBattleMons[gBattlerAttacker].estado), &gBattleMons[gBattlerAttacker].estado);
        MarcaCombatienteOcupado(gBattlerAttacker);
    }
    else
    {
...
static void Cmd_settorment(void)
{
    ...
    if (TransitorioActivo(gBattlerTarget, TRANSITORIO_TORMENTO))
    {
        gBattlescriptCurrInstr = cmd->failInstr;
    }
    else
    {
        PonTransitorio(gBattlerTarget, TRANSITORIO_TORMENTO);
...
static void Cmd_tryswapitems(void)
{
    ...
    else
    {
        ...
        *newItemAtk = gBattleMons[gBattlerTarget].item;
        ...
        {
            gResultadoCambioObjeto = OBJETO_AMBOS; // attacker's item -> <- target's item
        }
        else if (oldItemAtk == ITEM_NONE && *newItemAtk != ITEM_NONE)
        {
            ...
            gResultadoCambioObjeto = OBJETO_TOMADO; // nothing -> <- target's item
        }
        else
        {
            CheckSetUnburden(gBattlerAttacker);
            gResultadoCambioObjeto = OBJETO_DADO; // attacker's item -> <- nothing
        }
    }
}
...
static void Cmd_setgastroacid(void)
{
    ...
    PonTransitorio(gBattlerTarget, TRANSITORIO_BILIS);
    gBattlescriptCurrInstr = cmd->nextInstr;
}
...
static void Cmd_pontransitorio(void)
{
    CMD_ARGS(u8 transitorio, const u8 *failInstr);

    if (TransitorioActivo(gBattlerAttacker, cmd->transitorio))
    {
        gBattlescriptCurrInstr = cmd->failInstr;
    }
    else
    {
        PonTransitorio(gBattlerAttacker, cmd->transitorio);
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
}
    ...
    else
    {
        ...
        case ABILITY_NATURAL_CURE:
            ...
            ComandoFijaDatosPokemon(battler, REQUEST_STATUS_BATTLE,
                                         ...
                                         sizeof(gBattleMons[battler].estado),
                                         &gBattleMons[battler].estado);
            MarcaCombatienteOcupado(battler);
            break;
...
{
    if (!(TransitorioActivo(battlerDef, TRANSITORIO_SUSTITUTO)))
        return FALSE;
...
static void Cmd_EliminaPantallas(void)
{
    ...
    if (MovimientoEsEfectivo(gCombate->resultadoMovimiento) && (gSideTimers[lado].reflectTimer || gSideTimers[lado].lightscreenTimer || gSideTimers[lado].auroraVeilTimer))
    {
...
static void Cmd_handleballthrow(void)
{
    ...
    else
    {
        ...
        if (HayAlgunEstado(gBattlerTarget))
        ...
        if (odds >= 255) // Captura garantizada
        {
            ...
            gElegidorTextoMultiple = (CalculatePlayerPartyCount() == PARTY_SIZE) ? 0 : 1;
            return;
        }
...
{
    // Sandygast/Palossand (los únicos vetados en el juego original) no existen en esta Pokédex.
    return FALSE;
}
...
void BS_ItemCureStatus(void)
{
    ...
    u32 transitorio = TransitorioQueCura(gLastUsedItem);
    bool32 statusChanged = FALSE;
    struct Pokemon *party = GetSideParty(side);

    // Estados transitorios, solo si el Pokemon esta en el campo.
    ...
    if (transitorio != 0 && TransitorioActivo(battler, transitorio - 1))
    {
        QuitaTransitorio(battler, transitorio - 1);
        statusChanged = TRUE;
    }

    // Heal Status1 conditions.
    if (!HealStatusConditions(&party[gCombate->itemPartyIndex[gBattlerAttacker]], EstadoQueCura(gLastUsedItem), battler))
    {
        statusChanged = TRUE;
    }
...
void ScriptCombate_PalmaRauda(void)
{
    ...
    case PRIORIDAD_MOVIMIENTO_MUY_ALTA:
        if (GetBattlerTotalSpeedStat(gBattlerAttacker) > GetBattlerTotalSpeedStat(objetivo))
            gBattlescriptCurrInstr = cmd->nextInstr;
        else
            gBattlescriptCurrInstr = cmd->failInstr;
        break;
...
void IntentaRecuperarSaludTrasVencer(void)
{
        ...
        && !NoAliveMonsForEitherParty())
    {
        ...
        s32 maxHP = CuantosPSMaximos(combatiente);
        gBattleMoveDamage = (maxHP / 3) + gBattleMons[combatiente].hp;
