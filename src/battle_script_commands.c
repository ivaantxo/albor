...
void (* const gBattleScriptingCommandsTable[])(void) =
{
    ...
    BATTLE_CMD(counterdamagecalculator),
    BATTLE_CMD(mirrorcoatdamagecalculator),
    ...
    BATTLE_CMD(assistattackselect),
    BATTLE_CMD(trysetmagiccoat),
    ...
    else if (gMovesInfo[gCurrentMove].magicCoatAffected && !gBattleStruct->bouncedMoveIsUsed)
    {
        ...
        if (GetBattlerAbility(gBattlerTarget) == ABILITY_MAGIC_BOUNCE)
        {
            ...
            gBattleStruct->bouncedMoveIsUsed = TRUE;
              ...
              && GetBattlerAbility(ALIADO(gBattlerTarget)) == ABILITY_MAGIC_BOUNCE)
        {
...
static bool32 AccuracyCalcHelper(u16 move)
{
    ...
    else if (GetBattlerAbility(gBattlerAttacker) == ABILITY_NO_GUARD)
    {
        if (!JumpIfMoveFailed(move))
            RecuerdaHabilidadCombate(gBattlerAttacker, ABILITY_NO_GUARD);
        return TRUE;
    }
    ...
    {
        if (!JumpIfMoveFailed(move))
            RecuerdaHabilidadCombate(gBattlerTarget, ABILITY_NO_GUARD);
        return TRUE;
    }
...
u32 GetTotalAccuracy(u32 battlerAtk, u32 battlerDef, u32 move, u32 atkAbility, u32 defAbility, u32 atkHoldEffect, u32 defHoldEffect)
{
    ...
    if (defAbility == ABILITY_WONDER_SKIN && EsMovimientoDeEstado(move) && moveAcc > 50)
        moveAcc = 50;
    ...
    else
    {
        ...
        if (!PorcentajeAleatorio(accuracy))
        {
            ...
            if (gMovesInfo[gCurrentMove].effect == EFFECT_DRAGON_DARTS
                ...
                && !TargetFullyImmuneToCurrMove(gBattlerAttacker, ALIADO(gBattlerTarget)))
            {
...
s32 CalcCritChanceStageArgs(u32 battlerAtk, u32 battlerDef, u32 move, bool32 recordAbility, u32 abilityAtk, u32 abilityDef, u32 holdEffectAtk)
{
    ...
    if (critChance != -1 && (abilityDef == ABILITY_BATTLE_ARMOR || abilityDef == ABILITY_SHELL_ARMOR))
    {
        ...
        {
            if (critChance == -2)
                RecuerdaHabilidadCombate(battlerDef, abilityDef);
    ...
    else
    {
        ...
        if (MovimientoEsEfectivo(gCombate->resultadoMovimiento))
        {
            ...
            if (gBattleMons[gBattlerTarget].status2 & STATUS2_SUBSTITUTE)
                multihit = gContadorMultigolpes;
            else if (gContadorMultigolpes != 0 && gContadorMultigolpes != 1)
            {
    ...
    if (MovimientoEsEfectivo(gCombate->resultadoMovimiento) || (gHitMarker & HITMARKER_PASSIVE_DAMAGE))
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (EsMovimientoFisico(gCurrentMove) && !(gHitMarker & HITMARKER_PASSIVE_DAMAGE) && gMovesInfo[gCurrentMove].effect != EFFECT_PAIN_SPLIT)
                {
                ...
                else if (!EsMovimientoFisico(gCurrentMove) && !(gHitMarker & HITMARKER_PASSIVE_DAMAGE) && gMovesInfo[gCurrentMove].effect != EFFECT_PAIN_SPLIT)
                {
...
static void Cmd_resultmessage(void)
{
    ...
    else
    {
        ...
        {
        case MOVE_RESULT_SUPER_EFFECTIVE:
            if (!gContadorMultigolpes)  // Don't print effectiveness on each hit in a multi hit attack
            {
                // Signal for the trainer slide-in system.
    ...
    else
    {
        ...
        else
        {
            ...
            case MOVE_EFFECT_PAYDAY:
                ...
                if (GetBattlerSide(gBattlerAttacker) == LADO_JUGADOR && gSpecialStatuses[gBattlerAttacker].parentalBondState!= PARENTAL_BOND_2ND_HIT)
                {
                    ...
                    if (!((moveTarget == MOVE_TARGET_BOTH || moveTarget == MOVE_TARGET_FOES_AND_ALLY)
                        && GetNextTarget(moveTarget, TRUE) != NUMERO_COMBATIENTES))
                    {
                        BattleScriptPush(gBattlescriptCurrInstr + 1);
            ...
            case MOVE_EFFECT_FLAME_BURST:
                if (IsBattlerAlive(ALIADO(gBattlerTarget))
                        && !(gStatuses3[ALIADO(gBattlerTarget)] & STATUS3_SEMI_INVULNERABLE)
                        && GetBattlerAbility(ALIADO(gBattlerTarget)) != ABILITY_MAGIC_GUARD)
                {
...
static void Cmd_setmultihit(void)
{
    ...
    gContadorMultigolpes = cmd->value;
    gBattlescriptCurrInstr = cmd->nextInstr;
}
    ...
    do
    {
        ...
        case MOVEEND_RAGE: // rage check
            if (gBattleMons[gBattlerTarget].status2 & STATUS2_RAGE
                ...
                && !EsMovimientoDeEstado(gCurrentMove)
        ...
        case MOVEEND_CHOICE_MOVE: // update choice band move
            {
                ...
                if ((HOLD_EFFECT_CHOICE(holdEffectAtk) || GetBattlerAbility(gBattlerAttacker) == ABILITY_GORILLA_TACTICS)
        ...
        case MOVEEND_MOVE_EFFECTS2: // For effects which should happen after target items, for example Knock Off after damage from Rocky Helmet.
        {
            ...
            case MOVE_EFFECT_STOCKPILE_WORE_OFF:
                ...
                break;
            case MOVE_EFFECT_SMACK_DOWN:
                if (!EstaCombatienteEnSuelo(gBattlerTarget)
                 && IsBattlerAlive(gBattlerTarget)
        ...
        case MOVEEND_ATTACKER_VISIBLE: // make attacker sprite visible
            if (!(EsMovimientoEfectivo(gCombate->resultadoMovimiento))
    ...
    else
    {
        ...
        else
        {
            ...
            *(gBattleStruct->monToSwitchIntoId + battler) = PARTY_SIZE;
            ...
            if (battler == JUGADOR_IZQUIERDA && gBattleResults.playerSwitchesCounter < 255)
...
static bool32 DoSwitchInEffectsForBattler(u32 battler)
{
    ...
    if (gBattleMons[battler].ability == ABILITY_NEUTRALIZING_GAS && gSpecialStatuses[battler].announceNeutralizingGas == 0)
    {
        ...
        && EstaCombatienteEnSuelo(battler))
    {
...
{
    if (!gBattleStruct->itemLost[LADO_JUGADOR][gBattlerPartyIndexes[battler]].stolen
...
static void Cmd_various(void)
{
    ...
    case VARIOUS_RESET_PLAYER_FAINTED:
    {
        VARIOUS_ARGS();
        if (EsContraEntrenador()
            && IsBattlerAlive(JUGADOR_IZQUIERDA)
            && IsBattlerAlive(OPONENTE_IZQUIERDA))
        {
            gHitMarker &= ~HITMARKER_PLAYER_FAINTED;
        }
    ...
    case VARIOUS_PLAY_MOVE_ANIMATION:
    {
        ...
        BtlController_EmitMoveAnimation(battler, BUFFER_A, cmd->move, gBattleScripting.animTurn, 0, 0, gBattleMons[battler].friendship, &gDisableStructs[battler], gContadorMultigolpes);
        MarcaCombatienteOcupado(battler);
        gBattlescriptCurrInstr = cmd->nextInstr;
        return;
    }
    ...
    case VARIOUS_TRY_INSTRUCT:
    {
        ...
        if (move == MOVE_NONE || MoveHasAdditionalEffectSelf(move, MOVE_EFFECT_RECHARGE)
...
static u32 ChangeStatBuffs(s8 statValue, u32 statId, u32 flags, const u8 *BS_ptr)
{
    ...
    if (statValue <= -1) // Stat decrease.
    {
        if (gSideTimers[GetBattlerSide(battler)].mistTimer
            ...
            && !(battler == gBattlerTarget && GetBattlerAbility(gBattlerAttacker) == ABILITY_SIGILOSO))
        {
...
static void Cmd_forcerandomswitch(void)
{
    ...
    if (gBattleScripting.switchCase == B_SWITCH_RED_CARD
      && !(EsContraEntrenador())
      && GetBattlerSide(gBattlerAttacker) == LADO_OPONENTE)   // Check opponent's red card activating
    {
...
static void Cmd_tryinfatuating(void)
{
    ...
    if (GetBattlerAbility(gBattlerTarget) == ABILITY_OBLIVIOUS)
    {
        ...
        RecuerdaHabilidadCombate(gBattlerTarget, ABILITY_OBLIVIOUS);
    }
    else
    {
...
static void Cmd_transformdataexecution(void)
{
    ...
    else
    {
        ...
        RecuerdaHabilidadCombate(gBattlerAttacker, gBattleMons[gBattlerAttacker].ability);

        BtlController_EmitResetActionMoveSelection(gBattlerAttacker, BUFFER_A, RESET_MOVE_SELECTION);
        MarcaCombatienteOcupado(gBattlerAttacker);
        gMensajeBatalla = B_MSG_TRANSFORMED;
    }
}
...
static void Cmd_painsplitdmgcalc(void)
{
    ...
    if (!(DoesSubstituteBlockMove(gBattlerAttacker, gBattlerTarget, gCurrentMove)))
    {
        s32 hpDiff = (gBattleMons[gBattlerAttacker].hp + CuantosPSLeQuedan(gBattlerTarget)) / 2;
        s32 painSplitHp = gBattleMoveDamage = CuantosPSLeQuedan(gBattlerTarget) - hpDiff;
...
static void Cmd_healpartystatus(void)
{
    ...
    if (gCurrentMove == MOVE_HEAL_BELL)
    {
        ...
        if (IsBattlerAlive(partner))
        {
            ...
            {
                RecuerdaHabilidadCombate(partner, gBattleMons[partner].ability);
...
static void Cmd_jumpifnopursuitswitchdmg(void)
{
    ...
    if (gAccionElegida[gBattlerTarget] == B_ACTION_USE_MOVE
        ...
        && gMovesInfo[gMovimientoElegido[gBattlerTarget]].effect == EFFECT_PURSUIT)
    {
        s32 i;
...
static void Cmd_EliminaPantallas(void)
{
    ...
    if (EsMovimientoEfectivo(gCombate->resultadoMovimiento)
    && (gSideTimers[lado].reflectTimer
     || gSideTimers[lado].lightscreenTimer
     || gSideTimers[lado].auroraVeilTimer))
    {
...
static void Cmd_givecaughtmon(void)
{
    ...
    {
        u16 lostItem = gBattleStruct->itemLost[LADO_OPONENTE][gBattlerPartyIndexes[GetCatchingBattler()]].originalItem;
...
void BS_ItemCureStatus(void)
{
    ...
    if (gBattleStruct->itemPartyIndex[gBattlerAttacker] == gBattlerPartyIndexes[gBattlerAttacker])
    {
        ...
        gBattleMons[gBattlerAttacker].status2 &= ~GetItemStatus2Mask(gLastUsedItem);
                ...
                && gBattleStruct->itemPartyIndex[gBattlerAttacker] == gBattlerPartyIndexes[ALIADO(gBattlerAttacker)])
    {
        battler = ALIADO(gBattlerAttacker);
        previousStatus2 = gBattleMons[battler].status2;
...
void BS_ItemRestorePP(void)
{
    ...
    for (; i < loopEnd; i++)
    {
        ...
        if (pp != maxPP)
        {
            ...
            if (battler != NUMERO_COMBATIENTES
...
void BS_TryCopycat(void)
{
    ...
    if (gLastUsedMove == MOVE_NONE || gMovesInfo[gLastUsedMove].copycatBanned)
    {
        gBattlescriptCurrInstr = cmd->failInstr;
    }
    else
