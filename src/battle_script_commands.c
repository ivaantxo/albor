...
static inline u32 ObtenPosibilidadesGolpeCritico(u32 posibilidadesCritico)
{
...
s32 GetCritHitOdds(s32 critChanceIndex)
{
    if (critChanceIndex < 0)
        ...
        return ObtenPosibilidadesGolpeCritico(critChanceIndex);
}
...
s32 CalcCritChanceStageArgs(u32 battlerAtk, u32 battlerDef, u32 move, bool32 recordAbility, u32 abilityAtk, u32 abilityDef, u32 holdEffectAtk)
{
    ...
    if (critChance != -1 && (abilityDef == ABILITY_BATTLE_ARMOR || abilityDef == ABILITY_SHELL_ARMOR))
    {
        ...
        if (recordAbility)
        {
            ...
            else if (ObtenPosibilidadesGolpeCritico(critChance) == 1)
                RecuerdaHabilidadCombate(battlerDef, abilityDef);
        }
        critChance = -1;
    }
...
static void Cmd_critcalc(void)
{
    ...
    {
        gEsGolpeCritico = PorcentajeAleatorio(ObtenPosibilidadesGolpeCritico(critChance));
    }
    gBattlescriptCurrInstr = cmd->nextInstr;
}
...
static void Cmd_moveend(void)
{
    ...
    do
    {
        ...
        case MOVEEND_UPDATE_LAST_MOVES:
            ...
            if (gMovesInfo[gMovimientoElegido[gBattlerTarget]].effect == EFFECT_SHELL_TRAP
                && gBattlerTarget != gBattlerAttacker
...
static void PutMonIconOnLvlUpBanner(void)
{
    ...
    iconSheet.data = ObtenGraficosIconoPokemon(species, personality);
...
static void Cmd_tryKO(void)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (B_SHEER_COLD_ACC >= GEN_7 && gCurrentMove == MOVE_SHEER_COLD && !ES_COMBATIENTE_TIPO(gBattlerAttacker, TIPO_HIELO))
                odds -= 10;
        ...
        else
        {
            gMoveResultFlags |= MOVE_RESULT_MISSED;
            if (gBattleMons[gBattlerAttacker].level >= gBattleMons[gBattlerTarget].level)
                gMensajeBatalla = B_MSG_KO_MISS;
            else
                gMensajeBatalla = B_MSG_KO_UNAFFECTED;
            gBattlescriptCurrInstr = cmd->failInstr;
        }
    }
}
...
void BS_SetPledge(void)
{
    ...
    else if ((gAccionElegida[partner] == B_ACTION_USE_MOVE)
          && EsContraEntrenador()
...
void BS_TryUpperHand(void)
{
    ...
    if (GetBattlerTurnOrderNum(gBattlerAttacker) > GetBattlerTurnOrderNum(gBattlerTarget)
     ...
     || IS_MOVE_STATUS(gMovimientoElegido[gBattlerTarget])
