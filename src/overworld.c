...
void SetLastHealLocationWarp(u8 healLocationId)
{
    ...
    if (healLocation)
        SetWarpData(&gSaveBlockPtr->lastHealLocation, healLocation->mapGroup, healLocation->mapNum, WARP_ID_NONE, healLocation->x, healLocation->y);
}
...
void StoreInitialPlayerAvatarState(void)
{
    ...
    if (TestPlayerAvatarFlags(PLAYER_AVATAR_FLAG_BICI))
        sInitialPlayerAvatarState.transitionFlags = PLAYER_AVATAR_FLAG_BICI;
...
{
    u16 music = GetLocationMusic(&gSaveBlockPtr->location);
    return music;
}
