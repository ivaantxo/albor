...
static void TransferEggMoves(void)
{
    ...
    for (i = 0; i < DAYCARE_MON_COUNT; i++)
    {
        ...
        if (!GetBoxMonData(&gSaveBlockPtr->daycare.mons[i].mon, MON_DATA_SPECIES))
            continue;
        ...
        for (j = 0; j < numEggMoves; j++)
        {
            ...
            for (k = 0; k < DAYCARE_MON_COUNT; k++)
            {
                ...
                if (k == i || !GetBoxMonData(&gSaveBlockPtr->daycare.mons[k].mon, MON_DATA_SPECIES))
                    continue;
...
static u16 TakeSelectedPokemonFromDaycare(struct DaycareMon *daycareMon)
{
    ...
    GetBoxMonNickname(&daycareMon->mon, gVariableTexto1);
...
static u8 GetNumLevelsGainedForDaycareMon(struct DaycareMon *daycareMon)
{
    ...
    ConvertIntToDecimalStringN(gVariableTexto2, numLevelsGained, STR_CONV_MODE_LEFT_ALIGN, 2);
    GetBoxMonNickname(&daycareMon->mon, gVariableTexto1);
...
static u32 GetDaycareCostForSelectedMon(struct DaycareMon *daycareMon)
{
    ...
    ConvertIntToDecimalStringN(gVariableTexto2, cost, STR_CONV_MODE_LEFT_ALIGN, 5);
    return cost;
}
...
static u32 GeneraPersonalidadConNaturaleza(struct DayCare *daycare)
{
    ...
    u32 r = Random();
    return r - (r % NUMERO_NATURALEZAS) + naturalezaDeseada;
}

static void _TriggerPendingDaycareEgg(struct DayCare *daycare)
{
    daycare->offspringPersonality = GeneraPersonalidadConNaturaleza(daycare);
...
static void HeredaHabilidad(struct Pokemon *huevo, struct BoxPokemon *madre)
{
    ...
    {
        SetMonData(huevo, MON_DATA_ABILITY_NUM, &habilidadMadre);
    }
}
...
static void _GiveEggFromDaycare(struct DayCare *daycare)
{
    ...
    struct BoxPokemon *madre = &daycare->mons[parentSlots[0]].mon;
    struct BoxPokemon *padre = &daycare->mons[parentSlots[1]].mon;
...
static void EstableceDatosInicialesHuevoGuarderia(struct Pokemon *mon, u16 species, struct DayCare *daycare)
{
    ...
    SetMonData(mon, MON_DATA_MET_LEVEL, &nivelDeEncuentro);
}
...
// gVariableTexto1 = first mon's nickname
// gVariableTexto2 = second mon's nickname
// gVariableTexto3 = first mon trainer's name
static void _GetDaycareMonNicknames(struct DayCare *daycare)
{
    u8 otName[max(12, MAXIMO_CARACTERES_NOMBRE_JUGADOR + 1)];
    if (GetBoxMonData(&daycare->mons[0].mon, MON_DATA_SPECIES) != 0)
    {
        GetBoxMonNickname(&daycare->mons[0].mon, gVariableTexto1);
        GetBoxMonData(&daycare->mons[0].mon, MON_DATA_OT_NAME, otName);
        StringCopy(gVariableTexto3, otName);
    }

    if (GetBoxMonData(&daycare->mons[1].mon, MON_DATA_SPECIES) != 0)
    {
        GetBoxMonNickname(&daycare->mons[1].mon, gVariableTexto2);
    }
}

u16 GetSelectedMonNicknameAndSpecies(void)
{
    GetBoxMonNickname(&gPlayerParty[GetCursorSelectionMonId()].box, gVariableTexto1);
...
void SetDaycareCompatibilityString(void)
{
    ...
    StringCopy(gVariableTextoAmpliada, sCompatibilityMessages[whichString]);
}
...
static void DaycarePrintMonLvl(struct DayCare *daycare, u8 windowId, u32 daycareSlotId, u32 y)
{
    ...
    x = GetStringRightAlignXOffset(FUENTE_NORMAL, lvlText, 112);
