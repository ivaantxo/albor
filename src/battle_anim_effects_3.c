...
const struct SpriteTemplate gRelevoPokeballSpriteTemplate =
{
...
void AnimTask_RapinSpinMonElevation(u8 taskId)
{
    ...
    while (i <= task->data[3])
    {
        gRegistrosBuffersDistorsionFondo[0][i] = var2;
        gRegistrosBuffersDistorsionFondo[1][i] = var2;
        i++;
    }
...
static void RapinSpinMonElevation_Step(u8 taskId)
{
    ...
    while (i < task->data[1])
    {
        gRegistrosBuffersDistorsionFondo[0][i] = task->data[12];
        gRegistrosBuffersDistorsionFondo[1][i] = task->data[12];
        i++;
    }
    ...
    while (i <= task->data[3])
    {
        gRegistrosBuffersDistorsionFondo[0][i] = task->data[11];
        gRegistrosBuffersDistorsionFondo[1][i] = task->data[11];
        i++;
    }
...
static void AnimRelevoPokeball(struct Sprite *sprite)
{
...
void AnimTask_HideSwapSprite(u8 taskId)
{
    ...
    u8 *dest;
    u8 *src;
    u32 spriteId = gBattlerSpriteIds[gBattleAnimAttacker];
    ...
    case 0:
        ...
        break;
    case 1:
        GestionaCambioGraficoEspecie(gBattleAnimAttacker, gBattleAnimTarget, gBattleAnimArgs[1]);
...
void AnimTask_AcidArmor(u8 taskId)
{
    ...
    {
        gRegistrosBuffersDistorsionFondo[0][i] = bgX;
        gRegistrosBuffersDistorsionFondo[1][i] = bgX;
        gRegistrosBuffersDistorsionFondo[0][i + 1] = bgY;
        gRegistrosBuffersDistorsionFondo[1][i + 1] = bgY;
    }

    parametrosDistorsionFondo.bitsDMA = DISTORSION_FONDO_DMA_32;
    parametrosDistorsionFondo.estado = ESTADO_DISTORSION_FONDO_ACTIVO;
    EscribeParametrosDistorsionFondo(parametrosDistorsionFondo);
...
static void AnimTask_AcidArmor_Step(u8 taskId)
{
    ...
    case 0:
        ...
        {
            gRegistrosBuffersDistorsionFondo[gDistorsionFondo.srcBuffer][offset + 1] = (i - var2) + bgY;
            gRegistrosBuffersDistorsionFondo[gDistorsionFondo.srcBuffer][offset] = bgX + var3 + (gSineTable[sineIndex] >> 5);
        ...
        while (var0 >= 0)
        {
            gRegistrosBuffersDistorsionFondo[0][var0] = bgX + ANCHO_PANTALLA;
            gRegistrosBuffersDistorsionFondo[1][var0] = bgX + ANCHO_PANTALLA;
            var0 -= 2;
        }
...
static void CreateSweatDroplets(u8 taskId, bool8 lowerDroplets)
{
    ...
    for (i = 0; i < 4; i++)
    {
        u32 spriteId = CreateSprite(&gFacadeSweatDropSpriteTemplate, xCoords[i], yCoords[i & 1], task->tSubpriority - 5);
        if (spriteId != MAX_SPRITES)
        {
...
static void AnimTask_GlareEyeDots_Step(u8 taskId)
{
    ...
    case 0:
        ...
        if (++task->tTimer > 3)
        {
            ...
            for (i = 0; i < 2; i++)
            {
                u32 spriteId = CreateSprite(&gGlareEyeDotSpriteTemplate, x, y, 35);
                if (spriteId != MAX_SPRITES)
                {
                    if (i == 0)
...
void AnimTask_HelpingHandAttackerMovement(u8 taskId)
{
    ...
    if (EsContraEntrenador() == TRUE)
    {
        ...
        int partnerX = GetBattlerSpriteCoord(ALIADO(gBattleAnimAttacker), BATTLER_COORD_X);
...
static void AnimForesightMagnifyingGlass(struct Sprite *sprite)
{
    ...
    if (GetBattlerSide(sprite->data[7]) == LADO_OPONENTE)
        sprite->oam.matrixNum = ST_OAM_HFLIP;
...
void AnimTask_TeeterDanceMovement(u8 taskId)
{
    ...
    task->data[4] = GetBattlerSide(gBattleAnimAttacker) == LADO_JUGADOR ? 1 : -1;
