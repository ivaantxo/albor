...
static void Task_HofPC_DrawSpritesPrintText(u8 taskId)
{
    ...
    ConvertIntToDecimalStringN(gVariableTexto1, gTasks[taskId].tCurrPageNo, STR_CONV_MODE_RIGHT_ALIGN, 3);
    StringExpandPlaceholders(gVariableTextoAmpliada, gText_HOFNumber);

    if (gTasks[taskId].tCurrTeamNo <= 0)
        HofPCTopBar_PrintPair(gVariableTextoAmpliada, gText_PickCancel, FALSE, 0, TRUE);
    else
        HofPCTopBar_PrintPair(gVariableTextoAmpliada, gText_PickNextCancel, FALSE, 0, TRUE);
